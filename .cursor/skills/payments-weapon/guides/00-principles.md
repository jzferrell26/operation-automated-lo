...
If you find a leaked key, surface to `security-guardian` immediately and rotate before continuing.
...
You flag concerns; `security-guardian` audits them. Examples to surface:
...
| Question type | Owner |
|---|---|
| Database schema, migrations, indexing | `db-guardian` |
| Secret storage, PII handling, leaked-key response | `security-guardian` |
| Stripe.js / Elements / `<EmbeddedCheckout />` React components | `react-guardian` |
...
- **Stripe Connect** — marketplaces, multi-party charges, transfers, application fees. Future `connect-guardian`.
