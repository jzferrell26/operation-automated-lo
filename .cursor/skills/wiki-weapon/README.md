...
This directory holds everything the `wiki-guardian` Guardian needs to do its job. Organized into six layers: **guides** (procedural rules), **references** (cheat sheets loaded on demand), **templates** (page seeds copied per write), **examples** (worked invocations to mirror), **reports** (output shapes and past runs), **research** (audit trail for the guides).

> **Agent entry point:** [`guild/.cursor/agents/wiki-guardian.md`](../../agents/wiki-guardian.md) (repo-local). The agent reads files from this directory by path; it does not auto-load everything into context.
>
> **Brief:** [`guild/command-briefs/wiki-guardian-command-brief.md`](../../../command-briefs/wiki-guardian-command-brief.md)
> **Recon:** [`guild/command-briefs/research/2026-04-29-claude-obsidian-recon.md`](../../../command-briefs/research/2026-04-29-claude-obsidian-recon.md)
...
- Never touch `index.md`, `<type>/_index.md`, `log.md`, `hot.md`, `.guild/file-hashes.json` — TS driver owns global state.
...
- `library-guardian` writes module narratives at `library/knowledge-base/<module>/*.md`. wiki-guardian does not touch this folder. See [`guild/.cursor/skills/library-weapon/README.md`](../library-weapon/README.md).
- `quality-guardian` writes QA reports under `library/qa/` and feature/issue `reports/` folders. wiki-guardian does not touch these.
- The TS driver (Guild VS Code extension) owns `index.md`, `<type>/_index.md`, `log.md`, `hot.md`, `.guild/file-hashes.json` inside `library/knowledge-base/wiki/`. wiki-guardian writes per-page content only.
...
10. Never touch global state files. The TS driver reconciles `index.md`, `log.md`, `hot.md`, `<type>/_index.md`, and `.guild/file-hashes.json` after.
