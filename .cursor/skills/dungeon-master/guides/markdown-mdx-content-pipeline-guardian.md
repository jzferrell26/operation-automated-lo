# markdown-mdx-content-pipeline-guardian — Dungeon Master's Guide

The Dungeon Master routing skill's record of when to invoke `markdown-mdx-content-pipeline-guardian`. Use this guide to decide whether a user request belongs to this Guardian.

**Guardian:** [`ai-tools/agents/markdown-mdx-content-pipeline-guardian.md`](../../agents/markdown-mdx-content-pipeline-guardian.md)
...
If a request straddles `docs-site-guardian` and this Guardian (e.g., "set up Starlight with Shiki"), prefer `docs-site-guardian` for the platform decision and then hand off to this Guardian for the highlighting configuration.
...
- **Docs site setup** — `docs-site-guardian` selects the platform and hands off; this Guardian picks up to configure the highlighting and plugin chain for the chosen platform.
- **React/Next.js app build** — `react-guardian` owns the `mdx-components.tsx` component map; this Guardian configures the pipeline that feeds it compiled MDX.
- **Security review** — `security-guardian` performs the broader XSS audit; this Guardian configures `rehype-sanitize` and `DOMPurify` as the first-line sanitization layer.
