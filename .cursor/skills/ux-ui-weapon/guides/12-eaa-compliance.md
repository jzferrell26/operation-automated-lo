...
This guide tells the Guardian:

- what the EAA covers,
- who it applies to,
- the practical implication for the design system the Guardian governs,
...
The Directive lists specific products and services. The Guardian cares about the **digital surface** subset:
...
- **Consumer-grade computer hardware, smartphones, OS** (mostly out of the Guardian's design-review scope, but worth knowing).
- **Self-service terminals** — ATMs, ticket machines, check-in kiosks.

Beyond these, the Directive lists products too (TVs, telephony equipment, e-readers); the Guardian's review focus is the **digital service** layer.
...
- **Microenterprise exemption** (services only): fewer than 10 employees AND annual turnover or balance-sheet total under €2 million. Both conditions must be true. Most products this Guardian sees fail this exemption.

Practical implication: if the user's product reaches EU consumers, treat the EAA as in force. The Guardian does not gate on jurisdiction.
...
> **WCAG 2.2 AA is the EU legal floor for any digital service this Guardian governs.**

This is why `guides/11-wcag-2-2-baseline.md` is the WCAG 2.2 enforcement guide and this guide is the **why-it-matters-legally** guide. The Guardian does not certify legal compliance — that is a lawyer's job. The Guardian enforces the technical floor that makes legal compliance possible.
...
- **The design brief should declare WCAG 2.2 AA conformance** and note that `00-design-brief.md` references this guide. If it doesn't, the Guardian updates the brief first (per `guides/00-principles.md` rule #1).

---

## What the Guardian changes about its review behavior under EAA

Almost nothing changes operationally — the Guardian was already enforcing APG (`guides/00-principles.md` rule #4) and now WCAG 2.2 AA (`guides/11-wcag-2-2-baseline.md`). What changes is the **escalation tone**:

- A WCAG 2.2 AA failure in a product that reaches EU consumers is no longer "an accessibility nice-to-have." The Guardian writes the delta in `templates/review-output.md` shape and tags it with `EAA RISK` so the developer knows the legal exposure exists.
...
- **Microenterprise self-declaration**: if the product owner believes the microenterprise exemption applies, the Guardian does NOT validate that legally — the Guardian notes "exemption claimed; consult counsel" in the review and continues enforcing the technical floor anyway, because the exemption is narrow and is not the Guardian's job to certify.

---

## Where this lives in the design-system folder

The Guardian expects the design-system folder to carry these artifacts; if any are missing, it adds them on first invocation per the open-folder-first rule:
...
3. **Component spec accessibility section** in `03-components/<component>.md` already exists per `templates/component-brief-with-wrap.md` — the Guardian verifies it cites the matching APG pattern and any 2.2-relevant criterion.

---

## Penalties — context, not the Guardian's primary concern

For situational awareness only — the Guardian does not score legal exposure:
...
The Guardian's role is to make the technical floor solid. Legal exposure is the user's lawyer's job.
...
- The user requests a **certified compliance audit** (this Guardian doesn't certify; this is a paid audit-firm exercise).
...
*Caveat:* this guide is an engineering reference, not legal advice. The Guardian's job is to enforce the technical floor. Any legal conformance question — am I exempt, what does my member state require, do I need an Accessibility Statement — escalates out of the design-system loop.
