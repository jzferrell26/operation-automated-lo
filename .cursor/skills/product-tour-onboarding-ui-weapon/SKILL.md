---
name: product-tour-onboarding-ui-weapon
description: In-app product tour and onboarding UI specialist. Selects the right tour tool (Userpilot, Appcues, Userflow, Pendo Guides, Driver.js, Shepherd.js, Intro.js), implements tooltip/modal/hotspot/checklist components, wires segment-based trigger logic, and establishes a tour maintenance protocol that survives iterative UI changes. Invoke when the user says "set up a product tour", "build an onboarding checklist", "compare Driver.js vs Shepherd.js", "our tours keep breaking after deploys", "which product tour tool should we use", "add segment-based tour triggers", or "our tour is showing to the wrong users". Do NOT invoke for broader onboarding email sequences (lifecycle-email Guardian), user-auth flows (auth-guardian), design token work (ux-ui-guardian), or analytics instrumentation (posthog/mixpanel Guardians).
license: MIT
---
...
Do NOT load for:
- Onboarding email sequences (out of scope — no Guardian owns this yet).
- User authentication flows (auth-guardian).
- Design token system for tour tooltip styling (ux-ui-guardian).
- Analytics funnel instrumentation for tour events (posthog/mixpanel Guardians).
- Database schema for user-progress storage (db-guardian).
...
- **Route visual polish to `ux-ui-guardian`.** Tour tooltip/modal CSS must consume the product's design tokens; a parallel custom-CSS system in the tour layer is a maintenance trap.
- **Do not instrument analytics yourself.** Flag what needs tracking; route to the appropriate analytics Guardian.

---

## Research foundation

All factual claims in this weapon's guides trace to files in `research/external/`. The research index is at `research/index.md`. The summary of the five most influential sources is at `research/research-summary.md`. Refresh annually or on a major platform version bump.
...
*Forged by weapon-forge from `product-tour-onboarding-ui-guardian-command-brief.md` and `research/`. Part of the Guild AI Tools Factory.*
