...
- Checks voice and tone consistency against a supplied house style or Guild defaults (active voice, second person, present tense, imperative mood).
...
- (Optional) A style guide to enforce (URL or file path; defaults to Guild/Google style if not provided).
...
- This Guardian is proactive: it will volunteer for docs-related PRs without being named explicitly.
- When a house style guide is supplied, it overrides Guild defaults. The Guardian will enforce the supplied guide.
- Voice matching (ghostwriting to match an existing author's voice) is supported but flagged as best-effort.
- No Diataxis-specific Vale ruleset exists as of May 2026; Vale handles pattern-level style rules while this Guardian handles structural/mode-level classification.
