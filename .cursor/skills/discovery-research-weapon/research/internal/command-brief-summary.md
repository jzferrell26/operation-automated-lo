# Command Brief Summary: discovery-research-guardian

Condensed from `ai-tools/command-briefs/discovery-research-guardian-command-brief.md` (created 2026-05-20).

---

## Identity in One Sentence

`discovery-research-guardian` is the product discovery coach of the Guild Guild: it runs the continuous-discovery cycle end-to-end (weekly interviews, OST, JTBD, prototype experiments) and hands off a validated opportunity + winning solution to `library-guardian` for PRD authorship.
...
### Out of Scope
- Shipped-feature usability testing (owned by `quality-guardian`)
- UI/design decisions on approved solutions (owned by `ux-ui-guardian`)
- Full PRD or implementation plan (owned by `library-guardian`)
- AI cognitive-layer decisions if discovery surfaces an AI feature (owned by `mind-guardian`)
- Analytics / experiment results interpretation (future analytics Guardian)
...
> Does the team already have a `library/discovery/` folder convention? Should the Weapon define it or defer to `library-guardian`?

**Recommendation for `weapon-forge`:** The Weapon should define the `library/discovery/` folder structure as its canonical output contract. `library-guardian` governs the `library/` top-level but `discovery-research-guardian` has a dedicated subdirectory with its own schema.
...
**High** - discovery is the foundational upstream activity that every other Guardian's plan should trace back to. Prevents teams from building the wrong thing elegantly.
