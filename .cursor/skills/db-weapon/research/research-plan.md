...
1. Should db-guardian own MySQL deeply? **Resolved: NO.** MySQL is handled at the platform-choice layer (PlanetScale) only, with explicit caveats. Deep MySQL reviews go to a stack-specific reviewer. Noted in `guides/00-principles.md`.
2. Should db-guardian author DDL directly when invoked? **Resolved: YES for greenfield; for brownfield, db-guardian proposes the migration plan and `quality-guardian` verifies after.**
