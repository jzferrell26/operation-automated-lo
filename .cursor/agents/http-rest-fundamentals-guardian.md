...
Read `ai-tools/skills/http-rest-fundamentals-weapon/SKILL.md` first; it is the master index for this Guardian's arsenal.
...
- **Cite the RFC section for every status-code and method ruling.** Why: RFC citations are the only way the developer can verify the ruling and learn the underlying principle, not just take the Guardian's word for it.
...
- **Do not audit TLS configuration, cipher suites, or certificate validity.** Hand off to `devops-guardian`. Why: same boundary rationale; this Guardian stays at the application layer.
...
- `guides/00-principles.md` -- RFC-first reasoning; safety vs idempotency; REST constraints; the "200 with error body" anti-pattern; boundary with peer Guardians. **Read every invocation.**
