...
describe("PostgreSQL 17 adapter integration", { concurrency: false }, () => {
...
select current_setting('server_version_num')::integer as server_version_num
      ...
      assert.ok(versionRow.server_version_num >= 170000 && versionRow.server_version_num < 180000);
...
from pg_catalog.pg_prepared_statements
      ...
      await assert.rejects(
        ...
        (error) =>
          error instanceof PostgresAdapterError && error.code === "DB_PREPARED_STATEMENT_FORBIDDEN",
      );
    } finally {
...
insert into integration.outbox_events (
  ...
  it("resolves and isolates delivery claims for two tenants", async () => {
    ...
    try {
      ...
      await firstGuard.markCompletionUncertain(uncertainDelivery, "DELIVERY_COMPLETION_UNCERTAIN");
