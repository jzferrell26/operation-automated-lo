...
- **On demand** — when a human (or an orchestrator like a dm-skill) asks "check registry drift."
...
If you find **cross-guardian drift** — e.g., a UX/UI token used in code that has no `DesignTokenDefinition` catalog row, but `ux-ui-guardian`'s brief says it *should* exist — file the drift *and* ping `ux-ui-guardian` via a cross-link in the report. You do not fix cross-guardian drift alone.
