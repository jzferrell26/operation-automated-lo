...
| 5. Read the result | On the saved campaign page the sheet explains "Ready for approval" or "Needs changes" and, for each finding, what it means and how to fix it | `campaign.check.result`, `campaign.check.findings` | 20 s |
...
- **Multiple users, one workspace.** Progress is per `(location_id, user_id)`, so a seeded approver and a seeded creator each get their own journey; the approver's journey at step 6 approves the creator's campaign if one exists, otherwise their own.

### D6. Keyboard and screen reader

- The step panel is `role="dialog"` with `aria-labelledby` (the step title) and `aria-describedby` (the step body), `aria-modal="false"`: it does not trap focus, because the user must type into the highlighted field beneath it. Focus moves to the step title when a step opens and returns to the highlighted element when the user presses "Next" or "Continue"; when the step's action is a field, the "Continue" control is enabled only once the field is valid.
...
| 006C-AC-006 | Each of the seven steps renders the D3 copy, anchors to the D2 element, and performs its action; steps 2 and 3 write `setup_profile.v1`; step 4 prefills the draft from the profile and shows no demo default; step 5 explains the result and each finding; step 6 branches on `canApprove`; step 7 marks `completed`; integration tests cover every step in both branches. | 3 |
...
| 006C-AC-013 | Below 768 px the panel is a bottom sheet capped at 40 percent of the viewport, the anchored field is scrolled into view above it, and the sheet never covers the focused field; at 768 the panel anchors beside the element; proven by the 390 and 768 browser specs. | 3, 4 |
| 006C-AC-014 | The `review` Playwright project and the `pnpm test:db` step in D9 exist; the TLS terminator is `node:https` and `node:http` only; the run passes in the `database` CI job; the synthetic-mode `pnpm test:browser` in the `verify` job is unchanged. | 3 |
