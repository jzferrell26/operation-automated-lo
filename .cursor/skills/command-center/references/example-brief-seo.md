...
- Repo root path so the Guardian can read surrounding files for context
...
- If the Guardian cannot confidently classify a change (e.g., intentional noindex for a private page), flag it as a question rather than a blocker.
...
- Consider a "dry run" mode where the Guardian reports without posting to the PR.
- Q: Should the Guardian also validate structured data via schema.org's validator, or is that a separate Guardian?
