...
- `templates/` with reusable stubs the Guardian will fill in.
- `reports/README.md` and any pre-baked report shapes.

The skill is intentionally bounded: it does NOT touch the `research/` folder (that is `loremaster`'s territory) and it does NOT touch the Guardian file (that is `guardian-creator`'s).

## Inputs `dms-hand` passes to `weapon-forge`

`weapon-forge` reads from disk; `dms-hand` just needs to tell it WHERE to read. Pass:
...
The skill discovers the research folder automatically as a subdirectory of the weapon folder. It MUST refuse to proceed if `research/` is empty -- the older version of `weapon-forge` (pre-loremaster) would conduct its own research; the current version requires `loremaster`'s output to exist.
...
If `weapon-forge` ran before `loremaster`, it would have nothing to read in steps 2-4. The phase order is non-negotiable.
...
- **Guides reference files that do not exist** (e.g., a guide cites `templates/foo.md` but `templates/foo.md` was never written). Flag in the final report; broken internal references degrade Guardian performance.

## Implementation note for `dms-hand`

Like Phase 1, "invoking" `weapon-forge` from `dms-hand` typically means a skill load:
...
The full weapon-authoring logic is in `weapon-forge`'s skill content, not duplicated here. This guide's role is to specify the contract at the boundary: what `dms-hand` hands in, what `dms-hand` verifies coming out.
