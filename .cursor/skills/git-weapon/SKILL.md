...
The procedural arsenal for `git-guardian`. This weapon encodes the opinionated playbooks for every Git mastery surface: interactive rebase, conflict resolution, history rewriting, reflog recovery, worktrees, hooks, large-file storage, and submodule/subtree architecture.
...
**Not in scope:**
- CI/CD pipeline configuration using Git events → devops-guardian
- Server-side hook configuration in CI/CD runners → devops-guardian
- Credential rotation after a secrets-in-history incident → security-guardian
- Secret scanning policies and repository security tooling → security-guardian
...
The Guardian performs eight distinct actions. Each maps to a guide:
...
5. **Escalate to security-guardian for secrets-in-history scenarios.** Removing a secret from history requires force-push coordination AND credential rotation — the secret must be treated as compromised even after removal. That coordination (rotating keys, auditing access logs, notifying stakeholders) is security-guardian's domain.

6. **Escalate to devops-guardian for server-side hooks and CI Git configuration.** Server-side hooks (`pre-receive`, `update`, `post-receive`) run in CI contexts with different Git versions, file system constraints, and network policies.
...
└── research/                        (authored by loremaster — DO NOT MODIFY)
    ├── research-plan.md
    ├── research-summary.md
    ├── index.md
    ├── internal/
...
*Forged by `weapon-forge` from `git-guardian-command-brief.md` and research/. Part of the Guild AI Tools Factory by [Mario Aldayuz a.k.a @thenotoriousllama](https://github.com/thenotoriousllama).*
