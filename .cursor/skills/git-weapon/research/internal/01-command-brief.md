---
source_url: internal - git-guardian-command-brief.md
...
# Command Brief Summary: git-guardian

Structured summary of the Command Brief at `ai-tools/command-briefs/git-guardian-command-brief.md`. This is the internal design document that `weapon-forge` uses to understand what the Guardian does, what it does NOT do, and what playbook structure to build.

---

## Identity

- **Guardian name:** `git-guardian`
- **Weapon name:** `git-weapon`
...
## What git-guardian Does NOT Own

| Out of scope | Owned by |
|---|---|
| CI/CD pipeline configuration on top of Git events | `devops-guardian` |
| GitHub/GitLab API usage beyond the Git protocol | `devops-guardian` |
| PR review process tooling | `devops-guardian` |
| Server-side hook configuration in CI | `devops-guardian` |
| Credential rotation after secrets-in-history incident | `security-guardian` |
| Secret scanning policies | `security-guardian` |

---

## Seven Action Categories (Maps to Weapon Guides)

When a developer brings a Git problem, `git-guardian` maps it to one of:
...
5. **Escalate to the right Guardian.**
   - Secrets removal → secrets rotation → `security-guardian`
   - Server-side hooks + CI → `devops-guardian`

---

## Expected Weapon Structure
...
## Open Questions from Brief (for weapon-forge to address)

1. Should the weapon cover `git bisect` as a debugging tool, or is it out of scope?
2. Should branching strategy recommendations (trunk-based vs Git Flow) be a dedicated guide or folded into `00-principles.md`?

---

## Key Inputs the Guardian Needs from Developers
