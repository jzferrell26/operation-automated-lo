...
weapon: runbook-writing-weapon
retrieved_on: 2026-05-20
---

# Command Brief Decisions: runbook-writing-guardian

## Summary

The Command Brief defines `runbook-writing-guardian` as an opinionated operations writer that enforces three non-negotiable rules: (1) no-implied-context, (2) exact-command discipline, and (3) runbook-as-test mandate. The weapon (`runbook-writing-weapon`) must encode templates and guides that operationalize all three rules plus escalation-path architecture, rollback procedures, and postmortem linkage.

## Key decisions from the brief

### Runbook types (three canonical variants)

The Guardian classifies every incoming request into one of three runbook types before applying any template:
...
1. **Runbook automation scope**: Should the weapon cover runbook-as-code tools (Rundeck, AWS SSM)? The backlog purpose implies manual runbooks but 2026 tooling has blurred the line. Research suggests this is a real 2026 tension - see `SRE School (2026-02-15)` source for the automation maturity model.
2. **Overlap boundary with devops-guardian**: `runbook-writing-guardian` owns the document; `devops-guardian` owns the infrastructure knowledge that populates the commands. This boundary needs a concrete handoff example in the guides.

### Overlap boundaries (from NOTES section)

- `devops-guardian`: owns infrastructure decisions; `runbook-writing-guardian` surfaces a placeholder while the user decides
- `library-guardian`: owns broader documentation culture; `runbook-writing-guardian` owns runbook document format and testability
- If a runbook surfaces a missing deployment procedure, the Guardian surfaces it to `devops-guardian` and embeds a placeholder

## Annotations for weapon-forge

- The "done checklist" referenced in ACTION step 8 (`guides/00-done-checklist.md`) is mentioned but not fully specified - weapon-forge should derive it from the five critical directives plus research findings.
- The three runbook types map directly to the three templates; weapon-forge should align guide numbering and template naming.
- The five-minute rule is a hard constraint on template length - weapon-forge should enforce it structurally by keeping templates under ~1 page (600 words).
- The REFERENCE MATERIAL list includes PagerDuty response docs and Google SRE Book as primary anchors - weapon-forge should cite these in SKILL.md.
