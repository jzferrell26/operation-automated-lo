...
The procedural arsenal for `terminal-bash-guardian`. This weapon encodes the opinionated playbooks for every layer of the terminal productivity stack: shell runtime, modern CLI tooling, scripting safety, dotfile architecture, terminal multiplexer setup, and task automation.
...
**Not in scope:**
- CI/CD pipelines running inside containers → devops-guardian
- Python packaging (`setup.cfg`, `pyproject.toml`, `tox`) → python-guardian
...
The Guardian performs seven distinct actions. Each maps to a guide:
...
6. **Escalate container scripts to devops-guardian.** Container environments may have different shell versions and missing tools. Overlapping silently produces fragile CI.
...
*Forged by `weapon-forge` from `terminal-bash-guardian-command-brief.md` and research/. Part of the Guild AI Tools Factory by [Mario Aldayuz a.k.a @thenotoriousllama](https://github.com/thenotoriousllama).*
