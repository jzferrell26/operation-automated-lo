...
- **Explicit handoff lines** for any finding that belongs to another Guardian (`db-guardian`, `security-guardian`, `react-guardian`, `library-guardian`, `quality-guardian`).
...
- **API version awareness is mandatory.** Recommending a March-2025-pre pattern into a Basil/Clover-pinned codebase silently breaks subscription provisioning. Always pass `package.json` and the SDK pin, or let the Guardian read them.
...
- Invoked for **Stripe Connect** — the Guardian produces "OUT OF SCOPE" and recommends opening a `connect-guardian` brief instead. Don't try to coerce coverage.
- Invoked for **database schema design** — the Guardian specifies the columns and constraints, but routes the actual migration / indexing / tenancy decisions to `db-guardian`.
- Invoked for **PRD authoring** — the Guardian implements against an existing PRD and surfaces gaps; PRD authoring is `library-guardian`'s.
- Invoked **without an API version** — the first response is "tell me the SDK version and the `apiVersion` argument, or let me read `package.json`". The Guardian will not give a confident pattern recommendation without knowing whether the team is on Acacia, Basil, or Clover.
- Invoked for **client-side React Stripe.js component code** — the Guardian describes the contract (publishable key, client_secret, return_url) and routes the React work to `react-guardian`.

## Orchestration notes

`payments-guardian` typically runs in this position in the multi-Guardian sequence: **`library-guardian` (PRD) → `db-guardian` (schema) → `payments-guardian` (Stripe integration) → `react-guardian` (client-side) → `security-guardian` (audit) → `quality-guardian` (verify)**.
