...
The master routing skill for the Dungeon Master repository Cursor setup.

Dungeon Master does not perform work. It routes the primary Cursor agent's tasks to the correct Guardian (subagent) in the Guild, passing along the paired Weapon (skill) so every delegation arrives fully equipped.
...
Each Guardian has a dedicated, in-depth guide:
...
The Legendary Guardian Factory forges new Guardians end to end. To register a new Guardian with Dungeon Master after the Factory has produced the artifacts:
...
3. Update the multi-Guardian orchestration section in `SKILL.md` if the new Guardian fits an existing sequence.
...
*Part of the Guild curated by [Mario Aldayuz a.k.a @thenotoriousllama](https://github.com/thenotoriousllama).*
