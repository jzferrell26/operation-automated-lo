---
source_url: file://c:/Users/mario/GitHub/guild-code/ai-tools/command-briefs/hr-payroll-guardian-command-brief.md
...
# Internal Reference: hr-payroll-guardian Command Brief

## Summary
Command Brief authored by command-center on 2026-05-20. Defines the Guardian's identity, responsibility boundaries, expected input, action sequence, output format, and critical directives. Also contains the proposed guides/ structure, reference material list, and open questions that should inform weapon-forge's architecture decisions.
...
- **Out of scope:** General HRIS/performance management (Lattice, Rippling HRIS beyond payroll — no peer Guardian yet), recruiting/ATS, immigration/visa workflows beyond the EOR-vs-immigration fork
- **Handoff points (not overlaps):** auth-guardian (SSO/SCIM for payroll platform), db-guardian (HR data schema), library-guardian (PRD authorship), security-guardian (SSN/PII exposure in payroll integrations), payments-guardian (contractor invoice flows)
...
## Open questions from the brief (for weapon-forge to resolve)
...
3. Suggestion for worked example: "15-person US startup with 3 international contractors deciding between Gusto+Deel vs Rippling Global" — research gathered enough pricing data to construct this example; weapon-forge should include it in `guides/01-platform-selection.md` or `guides/03-international-eor.md`

## Annotations for weapon-forge
- The "classify before recommend" directive is the structural foundation of the weapon — all guides should funnel through `guides/02-classification-matrix.md` before reaching a platform recommendation
- The legal-advice fence is non-negotiable: the Guardian provides frameworks, not legal opinions. Every classification output should include a "consult a CPA / immigration attorney / employment lawyer at this branch" note
