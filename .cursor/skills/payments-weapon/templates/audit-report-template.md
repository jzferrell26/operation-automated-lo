...
`<Findings to surface to other Guardians with file:line.>`

- **`security-guardian`:** `<e.g., webhook secret stored in plaintext .env committed to repo at infra/dev.env:3>`
- **`db-guardian`:** `<e.g., processed_webhook_events table missing or not indexed>`
- **`react-guardian`:** `<e.g., publishable key embedded as raw string in src/components/Pricing.tsx:5 instead of env var>`
- **`library-guardian`:** `<e.g., no PRD covers what happens on failed dunning at day 21 — needs scoping>`
- **`quality-guardian`:** `<post-fix verification needed for items M1, M3, S2>`
...
## Acceptance checklist (hand to quality-guardian)
