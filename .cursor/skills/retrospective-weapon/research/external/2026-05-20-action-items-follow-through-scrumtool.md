...
- This is the primary source for `guides/04-action-items.md`. The five failure modes are the diagnostic checklist the Guardian uses when a team has low follow-through rate (ACTION 4).
- The four-component structure (owner, specific outcome, deadline, backlog placement) is the SMART-adjacent framework weapon-forge should encode as the action item template in `templates/action-items.md`.
...
- Contradictions: This source says deadline "within the sprint"; the Medium post (agile coach) says "the 3-question filter" is more important than SMART. Both are compatible — weapon-forge should present SMART as structure and the 3-question filter as a live facilitation check.
