...
The exact row format `dms-hand` appends to `ai-tools/proposed-guardians-completed.md` in Step 10c of `guides/09-close-out.md`.
...
- `<research-model-id>` is the model used by `loremaster`, lifted from the backlog metadata's `**Research Model:**` line.
...
- "Which model produced the most Guardians in the last 90 days?"
- "Did Guardians built with `gpt-5.5-extra-high` need more security findings than those built with `claude-opus-4-7-thinking-max`?"
...
- Position numbers are NOT necessarily monotonic (a high-position Guardian might complete before a low-position one if cycles run out-of-order, though `dms-hand`'s strict FIFO makes this unlikely).
- Rows are NEVER deleted from `completed.md`. The file is an append-only audit log.
- The file MAY have YAML frontmatter with optional fields like `date_updated`, `last_updated_by`, and `totals.rows`. If `totals.rows` is present, `dms-hand` should increment it during close-out.

## Failure variants (future work)

If `dms-hand` ever needs to log a permanently-abandoned cycle (e.g., a row that was determined to be a duplicate or otherwise unfit for forging), the suggested row format is:

```
NNN|guardian-name|abandoned|YYYY-MM-DD|reason:<short-reason>
```

This is NOT currently emitted by `dms-hand` (abandonment requires human decision; see F0.2 recovery in `guides/10-failure-modes.md`). It is reserved for a future maintenance Guardian.
