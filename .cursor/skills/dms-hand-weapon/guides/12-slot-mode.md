...
Slot mode lets the orchestrator run multiple dms-hand instances in parallel, each owning exactly one queue row, without any write races on the shared tracking files.
...
1. Dequeue the row from `proposed-guardians-queue.md` (delete the line, decrement `totals.rows`, bump `date_updated`, set `last_updated_by = dms-hand-orchestrator`).
2. Write `ai-tools/proposed-guardians-in-process-slot-NN.md` containing exactly one line: `NNN|guardian-name`.
...
Read `ai-tools/proposed-guardians-in-process-slot-NN.md`. Parse the single line `NNN|guardian-name`. This is the assigned row.
...
### Step 5 — Phase 1.5: loremaster
Dispatch via `Task(subagent_type="loremaster", ...)`. Wait for handoff. Verify `research/` is populated. On failure: write `slot-NN.failed` with `failed:loremaster`, preserve slot file, stop.
...
### Step 7 — Phase 3: guardian-creator
Load and run `guardian-creator` skill. Verify `ai-tools/agents/<guardian-name>.md` exists with valid frontmatter. On failure: write `slot-NN.failed` with `failed:guardian-creator`, stop.

### Step 8 — Write fragment files
Write all three fragments to `ai-tools/.batch-state/`:

**`slot-NN-roster-add.md`** — one complete table row for `ai-tools/skills/dungeon-master/SKILL.md`. Use the exact format from `templates/slot-roster-add-fragment.md`:
```
...
Delete `ai-tools/proposed-guardians-in-process-slot-NN.md`. Write `ai-tools/.batch-state/slot-NN.done` with a single line: `NNN|guardian-name|completed|YYYY-MM-DD`.
...
```
dms-hand slot-NN stopped. Awaiting orchestrator consolidation.
```
...
1. **Serial dm-registrar**: For each successful slot (has `.done`), read `slot-NN-roster-add.md` and invoke `dm-registrar` once. The fragment file contains exactly the text dm-registrar would produce. dm-registrar writes it to `ai-tools/skills/dungeon-master/SKILL.md` and creates `ai-tools/skills/dungeon-master/guides/<guardian-name>.md`.

2. **Append to completed log**: Read all `slot-NN-completion.md` fragments from successful slots. Append them to `ai-tools/proposed-guardians-completed.md` in one write.

3. **Flip backlog checkboxes**: Read each `slot-NN-backlog-flip.md` fragment. Apply each SEARCH→REPLACE pair to `proposed-guardians-backlog.md`.

4. **Clean up**: Delete all `slot-NN.*` files and the batch's `.batch-state/` contents. (If all 231 are done, delete `.batch-state/` entirely.)

5. **Surface failures**: For any slot with a `.failed` signal, log the row and failed phase. Do NOT append to completed or flip the checkbox. The row remains available for a manual re-run via a standard (non-slot) dms-hand invocation pointed at the preserved slot file.
...
| Failure | Recovery |
|---|---|
| `.failed` file present, slot file preserved | Manually re-run: `dms-hand slot=NN` pointing at the existing slot file, or convert to standard dms-hand run |
| Fragment files written but `.done` missing | Re-run the slot; idempotent writes to per-Guardian artifact paths |
| dm-registrar fails during serial phase | Retry just the dm-registrar call for that slot's roster-add fragment |
...
- Slot files: `ai-tools/proposed-guardians-in-process-slot-01.md` through `slot-99.md`
