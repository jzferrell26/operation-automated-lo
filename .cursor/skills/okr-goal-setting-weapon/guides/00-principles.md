...
Core operating principles for `okr-goal-setting-guardian`. Read before any other guide.
...
An **Objective** is an ambitious, qualitative direction for a time period. A **Key Result** is a measurable outcome that proves you moved in that direction. That's all. Everything else in this weapon is an elaboration on that contract.
...
- **`library-guardian`** owns storing OKR artefacts in `library/`. This Guardian authors the content; `library-guardian` owns the folder structure.
- **`agile-scrum-guardian`** owns Scrum ceremonies. When sprint goals intersect with OKRs, this Guardian owns the OKR side and notes the Scrum connection.
- **Domain Guardians** (`react-guardian`, `db-guardian`, etc.) own the engineering work OKRs point at. This Guardian writes the goal; the domain Guardian plans the execution.
