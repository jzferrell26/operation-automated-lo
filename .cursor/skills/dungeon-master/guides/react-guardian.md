...
It does not do visual design, SEO, or security audits. Those surface as flagged handoffs to the appropriate Guardian.
...
- Explicit handoff lines for any finding that belongs to another Guardian (security, SEO, UX).

## Critical directives to respect when routing

- **React version awareness is mandatory.** Recommending a React 19 pattern into a React 18 codebase is a silent-drift bug. Always pass `package.json` or let the Guardian read it.
- **Bleeding-edge is not reckless.** Novel patterns are labeled "experimental" with their source URL. Do not treat the Guardian's experimental flags as weakness — it is calibrating risk.
...
- Invoked for a non-React stack (Svelte, Vue, Solid) — the Guardian produces reduced-coverage output and flags "REDUCED COVERAGE"; route to a stack-specific reviewer instead.
- Invoked for visual review — the Guardian will surface the request and hand off to `ux-ui-guardian`.
