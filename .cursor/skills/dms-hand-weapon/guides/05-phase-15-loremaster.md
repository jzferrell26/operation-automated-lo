# 05 - Phase 1.5: loremaster

Step 6 of the Command Brief's ACTION list (and an interlude between Phase 1 and Phase 2). After the Command Brief is written and the weapon folder is scaffolded, invoke the `loremaster` subagent to conduct depth-calibrated research.
...
## What `loremaster` does

`loremaster` is the Phase 1.5 worker subagent. Its job is to download, summarize, and categorize 2026-current sources for the new Guardian/Weapon pair's domain, then file them into `ai-tools/skills/<weapon-name>/research/`. It does NOT author `SKILL.md` or guides; that is `weapon-forge`'s job.

The subagent's full contract lives at `ai-tools/agents/loremaster.md`. Read it before Phase 1.5 if you need the depth-tier rubric, the file shape, the tool-availability rules, or the handoff line format.
...
`loremaster` is implemented as a subagent because:

1. The research workload is large enough at `normal`, `deep`, and `extreme` tiers to benefit from context isolation.
2. The subagent can run with a different model than `dms-hand` (per the Command Brief's `research_model` field).
3. Research tool calls (Firecrawl, Exa) are bursty; isolating them avoids polluting `dms-hand`'s context with tool transcripts.

The other four phases (`command-center`, `weapon-forge`, `guardian-creator`, `dm-registrar`) are skills because their workloads are bounded, deterministic file-authoring tasks that benefit from shared context with `dms-hand`.

## Inputs `dms-hand` passes to `loremaster`

Construct a Task tool call with `subagent_type="loremaster"` and a prompt that contains:

- The Guardian name and Weapon name.
...
After `loremaster` completes, the following MUST exist:
...
`dms-hand` verifies (a) the four required files exist, (b) the subfolders contain at least one source file each (or one of them is empty with a documented reason in the summary), and (c) the handoff line is present in the subagent's response. If any check fails, STOP and route to `guides/10-failure-modes.md` under "loremaster failed."

## Depth tier and budget expectations

`loremaster`'s own contract documents the four depth tiers. As a `dms-hand` operator, you do not need to memorize them, but you should know roughly:

| Depth | Wall-clock | Files written | When to expect this tier |
|---|---|---|---|
| `shallow` | 5-15 minutes | 5-10 source files plus 3 metadata files | Procedural / orchestration weapons; well-bounded domains. |
...
If the depth tier is `deep` or `extreme`, expect `loremaster` to confirm acceptance with the caller in a short pre-flight message before starting. `dms-hand` should not interfere with that exchange; it is between the subagent and the user.
...
In the early factory pipeline (before `loremaster` was added in late 2025), `weapon-forge` did its own research. The separation makes the pipeline more debuggable:
...
The `weapon-forge` SKILL.md still has a "Step 3: Conduct research" header from before the split, but the inline instructions defer to `loremaster`. See `guides/06-phase-2-weapon-forge.md` for the implication: `weapon-forge` does NOT re-run research; it consumes what is already in `research/`.
