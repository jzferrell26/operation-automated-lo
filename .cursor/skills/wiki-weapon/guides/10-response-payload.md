...
- `notification_flags` → renders Cursor notifications in the Guild sidebar.
- `entities_detected` → updates `.guild/file-hashes.json` with `pages_created`/`pages_updated` per source file (delta-tracking key).
...
- `entities_detected` → `.guild/file-hashes.json`
