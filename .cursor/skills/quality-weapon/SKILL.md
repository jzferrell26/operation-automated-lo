...
- **Report, don't fix.** The Guardian identifies issues; it never implements fixes. That is the invoking developer's job (or another Guardian's).
- **Run after `security-guardian`, never before.** If invoked first, flag the ordering violation in the report and halt.

---

## Cross-Guardian relationships
...
- `examples/03-ordering-violation-escalation.md` — Guardian invoked before `security-guardian` ran; flags the violation and halts.
