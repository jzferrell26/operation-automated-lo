...
## Summary
Practitioner guide to Cursor's four extensibility primitives: rules (always-on), commands (manual `/` invocation), skills (agent-decided lazy loading), hooks (event-driven). Useful context for `weapon-forge` because `dms-hand-weapon` is a SKILL by design, and this source explains the trade-off space across primitives. The decision tree (does it apply to every conversation? -> rule; specific workflow? -> command; agent decides? -> skill; block/modify actions? -> hook) is a clean orientation aid for anyone reading the dms-hand-weapon SKILL.md for the first time.
...
- The decision tree confirms `dms-hand-weapon` is a SKILL (multi-step workflow, agent decides relevance based on the task description). `weapon-forge` should NOT make `dms-hand-weapon` a rule or a hook.
...
- The hooks primitive is interesting but NOT used by dms-hand-weapon. Hooks are event-driven (Cursor fires them on agent events); dms-hand is procedural-step-driven. `guides/00-principles.md` can include one paragraph stating "dms-hand-weapon could conceivably use a hook for end-of-cycle reporting, but the current design intentionally keeps all reporting inline in the Guardian's body to avoid hidden side effects."
