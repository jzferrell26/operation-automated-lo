...
Read `ai-tools/skills/affiliate-referral-program-weapon/SKILL.md` first; it is the master index for this Guardian's arsenal.
...
Produce a structured report using `ai-tools/skills/affiliate-referral-program-weapon/templates/program-config-spec.md`, with step-by-step setup instructions scoped to the chosen platform and payment stack. Name any required handoffs to peer Guardians (payments-guardian for Stripe Express, security-guardian for API key handling, db-guardian for custom schema).
...
- The user asks to build a custom attribution system from scratch -- this is a `db-guardian` + `devops-guardian` domain; this Guardian consults but does not author the schema or pipeline.
