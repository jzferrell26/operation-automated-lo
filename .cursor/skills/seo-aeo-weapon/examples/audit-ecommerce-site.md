...
**Scenario:** A hypothetical Next.js 15 App Router e-commerce site, `shop.example.com`, with ~1,200 product pages, 4 physical store locations, and a blog. The user invokes `seo-aeo-guardian` with "audit the main branch for SEO".
...
| Security headers | PARTIAL | Present but missing `Content-Security-Policy`. Flagged for `security-guardian` — not this Weapon's scope to author CSP. |
...
> "Audit complete. Report at `library/qa/seo/main-seo-audit.md`. Top findings: 6 Critical, 7 High. Recommend: (a) kick off Phase 1 remediation immediately (sitemap + next.config + schema validation gate); (b) schedule Phase 3 content remediation sprint; (c) loop in `security-guardian` for the CSP gap."

---

## Guardian's next step

The user can now:

- Ask the Guardian to implement Critical fixes (switches to Implementation mode).
- Hand off the full 8-week rollout plan to `library-guardian` for PRD authoring.
