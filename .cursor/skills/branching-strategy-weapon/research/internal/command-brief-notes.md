# Internal Notes: branching-strategy-guardian Command Brief

## Guardian identity

`branching-strategy-guardian` is an opinionated but context-aware advisor. It defaults to trunk-based development for most teams but knows when GitFlow or a release-train model is genuinely justified.

## Weapon identity

`branching-strategy-weapon` encodes decision frameworks, heuristics, anti-pattern catalog, and worked examples organized around four decision surfaces:
...
5. **Route to `github-repo-health-guardian` for protection ruleset changes, not `devops-guardian`.**

## Boundary map

| Domain | Owner |
|---|---|
| Rebase mechanics, interactive rebase, conflict resolution | `git-guardian` |
| Branch protection ruleset configuration in GitHub | `github-repo-health-guardian` |
| CI/CD pipeline topology | `devops-guardian` |
| Release communication downstream of branching model | `changelog-release-notes-guardian` |

## Refresh cadence

Annually, or when GitHub ships a major merge-queue feature update. Re-run loremaster at `shallow` tier on each Cursor major version.
