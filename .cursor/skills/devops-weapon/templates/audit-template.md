...
- [ ] `security-guardian` — {{if any secret-leak / CVE / token-scope concern surfaced}}
- [ ] `db-guardian` — {{if migration step needs authoring or schema-level concern flagged}}
- [ ] `library-guardian` — {{if pipeline change is large enough to warrant PRD authoring}}
- [ ] `quality-guardian` — {{post-implementation verification}}
- [ ] `react-guardian` — {{if Node version / workspace decisions affect the React app}}
...
*Produced by devops-weapon. See `SKILL.md` for methodology.*
