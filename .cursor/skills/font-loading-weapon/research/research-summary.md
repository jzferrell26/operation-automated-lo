# Research Summary: font-loading-weapon
...
"use only WOFF2" directive, and the `size-adjust` mention. `weapon-forge` should
...
1. **Safari `size-adjust` support in 2026:** fontpie's README shows Safari ❌,
   but `size-adjust` landed in Safari 17 (September 2023). `weapon-forge` should
...
2. **Next.js 15/16 `next/font` API changes:** The research confirms the
   ...
   behavior changes in Next.js 15/16. `weapon-forge` should check the Next.js 15
...
3. **`font-display` default behavior in 2026 browsers:** The `auto` value is
   ...
   behave like `block` (~3s). `weapon-forge` should verify if any 2025/2026
...
4. **Subfont tool:** The Command Brief mentions `subfont` as a subsetting
   ...
   did not specifically surface `subfont` documentation. `weapon-forge` should
...
- **https://nextjs.org/docs/app/api-reference/components/font** — the full API
  ...
  than covered here. `weapon-forge` should read the complete table.
- **https://capsizefitters.vercel.app/** — listed in the Command Brief as a
  reference but not found in search results as a guide. `weapon-forge` should
...
- **https://github.com/nicholasgasior/fontpie** — the Command Brief links to
  ...
  There may be a naming confusion; `weapon-forge` should verify the correct repo.

---

*Generated by loremaster on 2026-05-20.*
*Ready for weapon-forge.*
