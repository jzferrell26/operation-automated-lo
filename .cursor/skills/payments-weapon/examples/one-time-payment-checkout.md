...
- `db-guardian` — `orders`, `processed_webhook_events` schemas.
- `security-guardian` — RBAC on the customer-service refund endpoint.
- `library-guardian` — PRD defining what each `lookup_key` fulfills (download URL, course access, physical shipment).
