...
- **Caller:** invoked by `dms-hand` (pipeline-controller Guardian) via the `loremaster` subagent, Phase 1.5 of the Guild AI Tools Factory pipeline

## Domain framing

`dms-hand-weapon` is a procedural orchestration arsenal, not a knowledge-domain weapon. The Guardian's contract is FIFO file plumbing across four markdown tracking files (`proposed-guardians-queue.md`, `proposed-guardians-in-process.md`, `proposed-guardians-completed.md`, `proposed-guardians-backlog.md`) plus sequential dispatch of four worker skills and one worker subagent. The bulk of the substantive material is internal to this repo. External research is calibrating signal: how the wider Cursor / multi-agent community handles sequential dispatch, FIFO queues represented as files, and atomic move semantics.

## Initial queries (from command brief, authored by command-center / session-zero)
...
These are the source-of-truth contract surfaces `dms-hand` reads on every invocation. They are not "external research" but they ARE primary authoritative evidence and must be filed as research notes with `source_type: internal-repo` and `authority: official`:

1. `ai-tools/command-briefs/dms-hand-command-brief.md` -- the Command Brief itself.
2. `ai-tools/proposed-guardians-queue.md` -- producer/consumer contract surface (YAML frontmatter, `pickup_protocol`, `row_format`).
3. `ai-tools/proposed-guardians-backlog.md` (header only, lines 1 to 50) -- backlog tier structure and metadata block format.
4. `ai-tools/agents/session-zero.md` -- canonical producer Guardian; the file-plumbing-style mirror that `dms-hand` consumes the output of.
5. `ai-tools/agents/loremaster.md` -- worker subagent pattern that `dms-hand` dispatches in Phase 1.5.
