...
2. **Canary/synthetic monitoring** — staged rollout with live metrics (not a tool the Guardian can invoke, but a recommendation it should make).
...
For a review-time Guardian, #1 and #3 are the actionable ones.

## Dependency-based "at-risk surface" check (Guardian-usable)
...
Source for the "regression detection" portion of `guides/04-five-axis-evaluation.md` (Detrimental Patterns axis) and `guides/07-common-gaps.md`. The Guardian cannot run tests — it reviews statically. But it can and should:
