...
1. **Audit only. Never write to the repo.** The Guardian reads; it never modifies branch protection, CI files, or repository settings. All findings are phrased as recommendations, not automated fixes.
...
6. **Hand off secret scanning results to `security-guardian`.** Whether secret scanning is *enabled* is this Guardian's check; what leaked secrets *mean* is `security-guardian`'s job.
