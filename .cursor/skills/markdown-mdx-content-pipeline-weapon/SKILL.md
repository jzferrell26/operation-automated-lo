---
name: markdown-mdx-content-pipeline-weapon
...
Do NOT activate for:
- Platform selection (Docusaurus, Starlight, Mintlify) → `docs-site-guardian`
- React component architecture for MDX (`mdx-components.tsx` internals) → `react-guardian`
- Broader XSS audit beyond sanitization config → `security-guardian`
- SEO/AEO concerns about rendered pages → `seo-aeo-guardian`
...
7. **Route platform-selection to docs-site-guardian.** This weapon implements the highlighting and plugin config after the platform is decided; crossing the boundary produces contradictory guidance.
...
- Re-run `loremaster` at `shallow` depth if: Shiki releases v5, expressive-code releases a major, or Next.js App Router changes its MDX integration.
- The sanitization guides are stable; refresh only if a rehype-sanitize schema default changes.

---

*Part of the Guild AI Tools Factory. Paired Guardian: `markdown-mdx-content-pipeline-guardian`.*
