# Git Guardian — Dungeon Master's Guide

The Dungeon Master routing skill's record of when to invoke `git-guardian`. Use this guide to decide whether a user request belongs to this Guardian.

**Guardian:** [`ai-tools/agents/git-guardian.md`](../../agents/git-guardian.md)
...
- Optional: Git version (`git --version`) — the Guardian will check this itself for advanced features
- Optional: the specific branch name, commit sha, or error message when diagnosing a problem

If the goal is unclear, the Guardian will ask for clarification before proceeding — it will never guess on a potentially destructive operation.
