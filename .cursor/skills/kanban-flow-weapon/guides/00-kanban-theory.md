...
`kanban-flow-guardian` owns the Kanban method. It does NOT own:

- Scrum ceremonies (sprint planning, retrospectives, velocity): no peer Guardian today; surface to user.
- CI/CD pipeline flow (deployment throughput, lead time for changes in the DORA sense): `devops-guardian`.
- Database schema for storing flow metrics: `db-guardian`.
- Building custom Kanban tooling: `react-guardian` / `python-guardian`.
