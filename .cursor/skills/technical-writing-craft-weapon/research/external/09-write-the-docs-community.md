...
**Community resources:** Write the Docs maintains a curated list of published style guides including Google, Microsoft, Apple, GitLab, Digital Ocean, and many others. This list is valuable for the Guardian when a "house style" is referenced in an input but not provided.
...
- The "skimmable" principle maps to the Guardian's heading-review step: headings must enable navigation without reading the surrounding text.
- The community's style guide list (https://www.writethedocs.org/guide/writing/style-guides/) is a useful reference for the Guardian when a user provides a style guide name but not the document itself - the Guardian can look up the appropriate guide.
...
- Write the Docs conference talks (writethedocs.org/videos/) are a secondary source that weapon-forge could consult for worked examples of documentation review processes.
