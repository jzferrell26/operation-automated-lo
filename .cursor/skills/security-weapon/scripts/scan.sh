...
#
# Runs the checks a human or a grep can do — so the Guardian spends its reasoning
# on the judgment calls (IDOR, business logic, PCI architecture).
#
# Outputs land in reports/scan-output/. The Guardian reads them, dedupes with its
...
#
# Exit code is always 0 — the Guardian decides what's fatal.
