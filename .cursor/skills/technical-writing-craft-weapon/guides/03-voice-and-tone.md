...
**Important:** When a house style guide is supplied, enforce that instead of Guild defaults. Do not silently apply Guild defaults when a house style exists. Name the conflict and ask the user to clarify if the two systems contradict.

---

## Guild defaults (apply when no house style is supplied)
...
3. Use those rules, not Guild defaults, for the review.
4. If the house style is ambiguous or silent on a specific rule, apply the Guild default and note in the report which default was applied.
