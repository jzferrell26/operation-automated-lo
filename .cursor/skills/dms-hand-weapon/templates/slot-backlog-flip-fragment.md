...
**Purpose:** Contains the exact SEARCH/REPLACE pair the orchestrator applies to `ai-tools/proposed-guardians-backlog.md` to flip the entry's checkbox from `[ ]` to `[x]`.
...
Where `N` is the **unpadded** position number (as it appears in the backlog heading — the backlog uses `1.` not `001.`), and `guardian-name` is the kebab-case Guardian name.
...
3. Applies a `StrReplace` on `ai-tools/proposed-guardians-backlog.md` with that exact pair.
