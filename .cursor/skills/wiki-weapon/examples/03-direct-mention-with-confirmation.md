...
Git context: I'll request from the Guild driver via .guild/queue/ if it's running, otherwise shell to git directly.
Output location: library/knowledge-base/wiki/{entities,concepts,decisions,questions}/

This will be a partial scan — index.md and log.md won't update until the next Document or Update run via the Guild sidebar.
...
wiki-guardian writes to `.guild/queue/git-context-request-1745930400.json`:
...
Wait up to 2 seconds for `.guild/queue/git-context-response-1745930400.json` to appear.
...
This was a partial scan — index.md, log.md, hot.md, and the hash manifest will update on the next Document or Update run via the Guild sidebar (or whenever the driver next reconciles `.guild/queue/`).
...
6. Updates `.guild/file-hashes.json`.
...
For ANY of the following, use the Guild sidebar's Document/Update buttons instead:
