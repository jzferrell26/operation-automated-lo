...
Activate `code-review-pr-weapon` when the user or orchestrator invokes `code-review-pr-guardian` for any of:
...
The Guardian uses this taxonomy everywhere. It is derived from Google Engineering Practices, ARDURA, PanDev, and Pillai Infotech research. See `guides/00-principles.md` for the full decision tree.
...
Every PR description the Guardian produces contains these six elements. See `guides/01-pr-description.md` for the full guide and `templates/pr-description.md` for the fill-in template.
...
These questions survived the loremaster sweep. They are documented rather than guessed:

1. **Taxonomy naming:** Should the Guardian default to plain-English labels (`blocker:` / `suggestion:` / `nit:`) or emoji labels (🔴 / 🟡 / 💡)? Default: plain-English unless the team has an existing emoji convention.
...
3. **Review captain scope:** The review captain pattern works well for teams of 10+. For smaller teams, the Guardian should recommend shared triage rather than a dedicated role. See `guides/04-async-review.md`.
...
5. **"What did NOT change" format:** This section is novel to this Guardian (not found in research corpus). The six-element template treats it as a named H3 section. Teams may prefer an inline note style instead. See `templates/pr-description.md`.
