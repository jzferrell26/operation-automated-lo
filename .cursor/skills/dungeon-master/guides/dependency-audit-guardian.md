# Dependency Audit Guardian — Dungeon Master's Guide

The Dungeon Master routing skill's record of when to invoke `dependency-audit-guardian`. Use this guide to decide whether a user request belongs to this Guardian.

**Guardian:** [`ai-tools/agents/dependency-audit-guardian.md`](../../agents/dependency-audit-guardian.md)
...
- The question is about a specific package's API or usage → the relevant language or framework Guardian
...
- The Guardian will surface open questions before acting on Snyk pricing, OWASP Dependency-Check Java state, or Renovate Mend tier differences; do not prompt it to skip these flags
- The Guardian gates CI only on `high` and `critical` severity by default; do not override this without explicit user justification
