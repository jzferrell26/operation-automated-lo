...
- Escalate PII/GDPR questions to `security-guardian`. Candidate data is PII; GDPR right-to-erasure for applicants is outside this weapon's scope.
...
- Escalate HRIS configuration depth (beyond the ATS handoff interface) to a future `hris-guardian`.
...
- When `hiring-ats-guardian` is invoked by the orchestrator
