...
Read `ai-tools/skills/cold-outreach-weapon/SKILL.md` first — it is the master navigation layer with the routing table, critical directives, open questions, and cross-Guardian handoffs.
...
- **AE discovery calls, demo scripting, account expansion:** out of scope for this Guardian. Say so explicitly.
- **LinkedIn content strategy, paid acquisition:** out of scope. Say so explicitly.
- **Enterprise SDR team workflows (Salesforce sequences, large-scale ops):** this Guardian is calibrated for founder-led (0-2 person) outreach. Flag that the playbook may need adjustment for enterprise SDR teams.
