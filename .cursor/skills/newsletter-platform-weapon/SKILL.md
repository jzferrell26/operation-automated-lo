---
name: newsletter-platform-weapon
...
- **Defer Stripe billing integration to `payments-guardian`.** Platform-native paid tiers are in scope; custom Stripe-on-top billing is not.
...
Route infrastructure-level DNS work (SPF/DKIM/DMARC records) to `devops-guardian` or the `resend` weapon.
...
*Forged by weapon-forge from `ai-tools/command-briefs/newsletter-platform-guardian-command-brief.md` and `research/`. Part of the Guild AI Tools Factory.*
