...
**Purpose:** Contains the exact line the orchestrator appends to `ai-tools/proposed-guardians-completed.md` to record the successful forging of this Guardian.
...
- `guardian-name` — kebab-case Guardian name
...
### Example (terminal-bash-guardian)

```
018|terminal-bash-guardian|completed|2026-05-20|research:gemini-3.5-flash|analyst:claude-4.6-sonnet-medium-thinking|builder:gpt-5.3-codex-xhigh
```
...
3. Appends all collected lines to `ai-tools/proposed-guardians-completed.md` in one write.
...
Slots that write `.failed` instead of `.done` do NOT get a completion entry. Their row stays `[ ]` in the backlog and they are excluded from the append step. They can be re-run manually with a standard dms-hand invocation.
