...
## Domain scope
The Guardian owns the full data-exchange surface: file ingestion → parse → column mapping → validation → error reporting (import side); workbook construction → streaming export (export side). Does NOT own: file drop zone UI (`ux-ui-guardian`), bulk-insert DB performance (`db-guardian`), upload endpoint security (`security-guardian`).
