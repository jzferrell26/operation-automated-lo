...
- Background jobs triggered by queue messages without a time component (no Weapon for this yet — handle inline)
- CI/CD pipeline design (that is `devops-guardian`)
- Database schema design for job metadata tables (that is `db-guardian`)
...
These are non-negotiable. Each guide in this weapon is structured around them.
...
The `research/` folder was populated by `loremaster` at `normal` depth (2025-11-20 to 2026-05-20). Key findings:
...
- `research/research-summary.md` lists 5 open questions for `weapon-forge` — see that file for details.

---

*Forged from `ai-tools/command-briefs/cron-scheduling-guardian-command-brief.md` and `research/`. Part of the Guild AI Tools Factory by [Mario Aldayuz a.k.a @thenotoriousllama](https://github.com/thenotoriousllama).*
