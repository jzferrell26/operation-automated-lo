...
# crm-integration-weapon

CRM connectivity playbook for `crm-integration-guardian`. Synthesized from 10 source notes covering Merge.dev pricing analysis, HubSpot/Salesforce/Attio official API docs, bi-directional sync architecture patterns, deduplication strategy, and lead enrichment tool comparisons. Research window: November 2025 -- May 2026.

See `research/research-summary.md` for key findings and open questions. The six-guide structure maps directly to the six major decision surfaces the guardian faces.
...
Do NOT load for:
- Cold email sequencing or deliverability (route to `cold-outreach-guardian`).
- The product's own internal Person/Account schema (route to `db-guardian`).
- Implementation of the sync backend in Django or Node.js (route to `python-guardian`).
- Frontend CRM sync widget or CRM data display UI (route to `react-guardian`).
- GDPR data residency for CRM data (flag and route to `security-guardian`).
...
+- research/                              (owned by loremaster -- do NOT modify)
   +- research-plan.md
   +- research-summary.md
   +- index.md
   +- internal/
...
*Forged by `weapon-forge` from `crm-integration-guardian-command-brief.md` and `research/`. Part of the Guild AI Tools Factory by [Mario Aldayuz a.k.a @thenotoriousllama](https://github.com/thenotoriousllama).*
