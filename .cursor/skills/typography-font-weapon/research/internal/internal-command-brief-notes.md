...
url: ai-tools/command-briefs/typography-font-guardian-command-brief.md
retrieved: 2026-05-20
---

# Command Brief Notes: typography-font-guardian

## Summary

The `typography-font-guardian` Command Brief defines an opinionated type-and-font specialist that owns the full technical typographic surface of web products. It covers font loading strategy selection, variable font configuration, fluid type scale construction via `clamp()`, vertical rhythm, and a reusable font-token layer.

The Guardian activates after `design-system-guardian` has chosen the typeface and before `ux-ui-guardian` applies tokens in components.

## Scope boundary (critical for weapon-forge)
...
- **Does NOT own:** typeface aesthetic selection (`design-system-guardian`), per-component token application (`ux-ui-guardian`), build pipeline font optimization steps (`devops-guardian`), font preference DB schema (`db-guardian`).
