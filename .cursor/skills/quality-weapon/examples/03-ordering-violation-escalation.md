...
Demonstrates the Guardian's behavior when it detects it was invoked before `security-guardian` ran. The Guardian does NOT proceed with a full audit — it writes a short ordering-violation report and halts.
...
The ordering constraint is absolute per the `quality-guardian` principles (see the Guardian's guide `00-principles.md`). "In a hurry" is the most common reason security is skipped, and it's also the most common reason post-deploy security incidents happen. Running QA on an un-securitied implementation produces a false-confidence green scorecard that masks real risk. If a security review is genuinely blocked (e.g., `security-guardian` itself is broken), escalate that — don't route around it.
...
- **Halt instead of audit.** `guides/00-principles.md` rule #6 says: "If the Guardian detects it was invoked first, it flags the ordering violation and recommends re-running after security fixes land." This is that behavior.
...
- **Does not override the user.** The user asked to skip security. The Guardian refused, kindly but firmly. This is one of the few places the Guardian overrides user instructions — and the reason is in the report.
