...
The N+1 query problem: one query fetches a list of N parent records, then N additional queries fetch related data — one per parent — instead of a single batched query. This is the single most common performance regression in ORM-backed Next.js apps and a leading Detrimental Pattern the Guardian should flag.
