...
Load this weapon when `status-page-guardian` is invoked. Typical triggers:
...
Do NOT load for:
- Monitoring/alerting infrastructure configuration → `devops-guardian`
- On-call rotation design, PagerDuty/OpsGenie setup → `devops-guardian`
- Observability dashboards, SLO/SLI definitions → `devops-guardian`
- Operational runbooks → `runbook-writing-guardian`
...
+- research/
   +- research-summary.md               (executive summary from loremaster)
...
| Paired Guardian | `ai-tools/agents/status-page-guardian.md` |
| Command Brief | `ai-tools/command-briefs/status-page-guardian-command-brief.md` |
...
*Forged by `weapon-forge` from `status-page-guardian-command-brief.md` and `research/`. Part of the Guild AI Tools Factory by [Mario Aldayuz a.k.a @thenotoriousllama](https://github.com/thenotoriousllama).*
