...
**Guardian:** [`ai-tools/agents/okr-goal-setting-guardian.md`](../../agents/okr-goal-setting-guardian.md)
...
`okr-goal-setting-guardian` is the Guild's OKR methodology expert. It owns the full OKR lifecycle:
...
- The user needs company strategy authored — that is an executive responsibility; this Guardian translates strategy into OKRs but does not create it.
- Sprint goal setting, backlog prioritization, or Scrum ceremony coaching is needed — route to `agile-scrum-guardian`.
- Engineering roadmap planning or feature prioritization — route to the relevant domain Guardian (`react-guardian`, `python-guardian`, etc.) or `library-guardian` for PRD authorship.
...
- **Quarterly planning sequence:** `okr-goal-setting-guardian` authors the OKRs → `agile-scrum-guardian` designs the Sprint Goals that support the OKRs → domain Guardians (`react-guardian`, `python-guardian`, etc.) own the engineering work the OKRs point at → `library-guardian` stores the OKR artefacts in `library/`.
