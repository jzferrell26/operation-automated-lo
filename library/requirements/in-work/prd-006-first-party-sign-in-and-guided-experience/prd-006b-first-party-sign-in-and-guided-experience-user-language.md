...
| 006B-AC-007 | Every route error code the campaign and auth handlers export has an entry in `user-messages.ts`; the create page, the approval control, the six auth forms, and the guided-setup panel render the mapped sentence, never the code; the unmapped fallback renders with a support reference on all four surfaces; a unit test asserts total coverage. (amended 2026-09-21, see Amendments) | 2 |
...
- **2026-09-21, D5, D6 and 006B-AC-002, the source guard's roots widen to `packages/application/src`, and two temporary exclusions.** Text said: D6's source guard reads only `apps/web/src/app/**`, `apps/web/src/features/**`, `apps/web/src/server/authenticated-workspace-data.ts`, `apps/web/src/server/runtime-authentication.ts`, `apps/web/src/server/email/**`, and `packages/ui/src/components/**`; D5's inventory names no file under `packages/application/src`. Code does: `tooling/tests/unit/user-language/forbidden-vocabulary.test.ts`'s `SCANNED_ROOTS` was widened, 2026-09-20 (commit `c0b5097`), to add `packages/application/src`, because `packages/application/src/campaign-workspace-read.ts`'s `label` fields carried the campaign next-step sentences ("Provider publication is not authorized.", and the rest) one layer below the component that renders them, so a word D2 bans in the component was still legal in the package that generated it. The widening came with two dated, single-file `EXCLUDED` entries (`campaign-workspace-read.ts`, `reporting.ts`), each with a stated removal condition. Why: closes a real gap the guard's stated globs left open; recorded here because D6's own text still lists only the six original roots. Touches: 006B-AC-002.
- **2026-09-21, D5, 006B-AC-002 and 006B-AC-008, the campaign next-action labels move into `apps/web/src/copy/user-language.ts`; the `campaign-workspace-read.ts` exclusion is now stale.** Text said: D5's inventory has no row for a copy module holding campaign next-action or check-result sentences; `packages/application/src/campaign-workspace-read.ts`'s temporary `EXCLUDED` entry (added 2026-09-20, above) names its removal condition as "once that move lands." Code does: Wave 7t's react lane (commit `e57a2fd`) removed every `label` field from `campaign-workspace-read.ts`; `CampaignNextActionId` gained an `already_decided` member (replacing a `approve_version` id that used to carry two different sentences); the five sentences now live in `apps/web/src/copy/user-language.ts`'s `CAMPAIGN_NEXT_ACTION_LABELS` map, keyed by that id, and `apps/web/src/features/campaigns/components/persisted-campaign-screen.tsx` and `open-house-draft-builder.tsx` read the label through the map instead of the application layer. The same module also carries `CHECK_RESULT_NEEDS_CHANGES = "Needs changes"` and `CHECK_RESULT_READY`, with `preflight_failed` mapped to `CHECK_RESULT_NEEDS_CHANGES`, matching D3's "preflight passed / blocked -> Ready for approval / Needs changes" row. Why: this is the removal condition the exclusion itself named. **Orchestrator action needed:** the `EXCLUDED` entry for `packages/application/src/campaign-workspace-read.ts` in `forbidden-vocabulary.test.ts` is stale as of `4e7ae09` (the file it excuses no longer holds any forbidden string; a `SCANNED_ROOTS` scan of it today would pass clean) and should be removed; the `reporting.ts` entry is untouched and still applies, that file was not part of this migration. `apps/web/src/copy/user-language.ts` should also be added to D5's inventory and to this sub-PRD's "Files expected to change" list. Touches: 006B-AC-002, 006B-AC-008; D5 (new copy source not yet listed).
- **2026-09-21, D7 and D8, and 006B-AC-007, the support reference reaches four surfaces.** Text said: 006B-AC-007's own pre-amendment wording named "the create page and the approval control" as the two places the mapped sentence and the unmapped-fallback support reference render (corrected above); D7's prose is broader ("the create page and the approval control render the mapped sentence") but D8 always scoped "Details for support" to wherever a reference could show, not to two named surfaces. Code does: `apps/web/src/features/shell/components/support-details.tsx`'s new `SupportReference` component (rendering the D8 region with just the support-reference row, exactly when the refusal's code has no mapped sentence) is used on four surfaces: the campaign create page (`open-house-draft-builder.tsx`), the approval control (`campaign-approval-controls.tsx`), all six auth forms through `useAuthSubmit`/`authProblemFor` (`auth-feedback.tsx`, `use-auth-submit.ts`), and the guided-setup panel (`guided-setup-provider.tsx`'s `writeFailure` state, rendered by `guided-setup-step.tsx`). Landed across Wave 7t (`f87d69c`, `e57a2fd`), carried forward unchanged by `79edb3e`. Why: a refusal on a guided-setup write or an auth submit previously showed the generic sentence with nothing to quote; D7 and D8 already promised the reference wherever a generic sentence could render, and AC-007's old wording undersold it. Touches: 006B-AC-007.
