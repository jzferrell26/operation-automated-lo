...
- The three-stage Maildir lifecycle (tmp -> new -> cur) is structurally identical to dms-hand's three-stage lifecycle (queue -> in-process -> completed). Document this mapping in `guides/00-principles.md`:
  - Maildir `tmp/` -> nothing direct (dms-hand doesn't have a "draft" stage; the row is born in `queue.md` by `session-zero`).
  - Maildir `new/` -> `proposed-guardians-queue.md` (visible to all, claimable by any consumer).
  - Maildir `cur/` -> `proposed-guardians-in-process.md` (exclusively owned by one consumer at a time).
  - Maildir delete-after-cur (no equivalent dir, file is in cur) -> `proposed-guardians-completed.md` (archive after work is done).
- The NFS caveat (link is atomic, rename is not) does NOT apply to dms-hand because the queue files live in a single working directory on a local filesystem and the move semantics are at the agent layer (markdown editing), not at the OS layer. Document this difference in `guides/00-principles.md` so the user doesn't expect Maildir-grade NFS guarantees.
- The stale-lock recovery guidance maps directly to dms-hand's "Refuse to start a new cycle if proposed-guardians-in-process.md already contains a row" rule. The in-process row IS a potential stale lock if a prior cycle crashed. `guides/10-failure-modes.md` should reference Maildir's stale-lock cleanup as prior art for the recovery procedure: examine the row, decide if work was completed (look for the artifacts on disk), then either move the row to completed.md manually or back to queue.md for a re-run.
