...
| **Phased rollout** | "plan the 8-week SEO rollout", "map this to a PRD" | Hand off the phase-by-phase plan to `library-guardian` when the deploying product wants a feature PRD (lands at `library/requirements/features/feature-<###>-<title>/prd-feature-<###>-<title>.md`); otherwise deliver inline. |
...
Deterministic checks the Guardian can run. These are starting points — extend per project.
...
Reports go to the host repo's `library/` tree, never to this Weapon. When producing an audit report, copy `templates/audit-report-template.md` and fill it in, saving the result at:
...
6. **Next.js version awareness.** Confirm `next` version on first contact. App Router + Metadata API requires 13.4+; `viewport` export split requires 14+. Flag Pages Router codebases as degraded coverage and hand off App Router migration to `react-guardian`.
...
- `security-guardian` — CSP headers overlap with `next.config.js > headers()`. Any change that adds or relaxes a `Content-Security-Policy` directive routes through `security-guardian` for the security pass before merge.
- `react-guardian` — Next.js App Router patterns (server components, route handlers, Server Actions). Pages Router → App Router migration is `react-guardian`'s territory; the Guardian flags the need and hands off.
- `library-guardian` — PRD authoring for phased SEO rollouts when the deploying product wants a feature PRD. The Guardian produces the phase plan; `library-guardian` formats it into a PRD at `library/requirements/features/feature-<###>-<title>/prd-feature-<###>-<title>.md`.
- `quality-guardian` — runs against any implementation the Guardian authors; verifies that SEO changes don't regress core user flows.

## Refresh cadence

SEO and AEO drift fast. This Weapon's `research/` folder must be reviewed every 90 days. Last forge: 2026-04-24. Next review: 2026-07-24. See `research/refresh-cadence.md` for the refresh protocol — the Guardian activates it on phrases like "refresh the SEO research" or "run the 90-day review".

## Out of scope

- Marketing copy. Keyword research. Editorial work. (Content Guardian's job.)
- Non-Next.js stacks. Nuxt, SvelteKit, Astro, plain HTML — flag and degrade coverage; do not pretend fidelity.
- Pages Router (Next.js ≤13.3). Flag and hand off migration to `react-guardian`.
- Paid-search and ads (Google Ads, Meta). Off-SEO.
- Backlink acquisition strategy. The playbook's Phase 8 (Link Building) is listed in `guides/10-implementation-phases.md` as informational only; the Guardian does not execute outreach.
