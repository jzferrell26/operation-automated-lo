---
name: session-zero-weapon
description: Equips the session-zero subagent with the canonical rubric, format, and routing logic for proposing brand new Guardians (Cursor IDE subagents) into the Guild AI Tools Factory. Use whenever a new guardian needs to be added to `ai-tools/proposed-guardians-backlog.md` plus `ai-tools/proposed-guardians-queue.md`. Encodes the four-tier research depth rubric, the three-role model routing (Research / Analyst / Builder), the category overrides (math_science, cli_devops, self_hosted_open), the exact backlog entry shape, the exact queue row shape, the search-query authoring discipline, and the position-numbering invariants. Not for editing existing entries, claiming queue rows, or running downstream pipeline steps (command-brief, weapon-forge, create-guardian, dm-register); those are owned by other skills and agents.
---

# session-zero-weapon

Authoring rubric for new Guardian proposals. The matching agent is [`session-zero`](../../agents/session-zero.md).

The Big Bang pair sits at the front of the Guild AI Tools Factory pipeline:

```text
session-zero (this Guardian's caller)
  -> proposes a guardian
  -> appends entry to proposed-guardians-backlog.md  (full shape)
  -> appends row   to proposed-guardians-queue.md     (NNN|name)
  -> downstream pipeline picks the queue row
     -> command-brief -> weapon-forge -> create-guardian -> dm-register
```
...
|---|---|
| `ai-tools/proposed-guardians-backlog.md` | New entry block under the appropriate Tier heading (or under the trailing Tier 17 Extended Coverage section if no clean tier fit). |
| `ai-tools/proposed-guardians-queue.md` | New `NNN\|guardian-name` row appended to the bottom of the body (after the YAML frontmatter and after the last existing row). |

It never touches `ai-tools/proposed-guardians-completed.md` (that file is owned by the downstream pipeline when a row is claimed) and it never touches `ai-tools/model-comparison-matrix.md` (that file is the source of truth for model selection and is read-only from this skill's perspective).
...
2. **Scope boundary** in one sentence. What this Guardian owns, and what it explicitly does NOT own (route-to clauses). The Guild convention is to hand off neighbouring concerns to peer Guardians rather than absorb them.
3. **Failure mode** in one sentence. What happens if this Guardian does its job badly. This signals the right depth tier.
4. **Stack context** if any. Named platforms, libraries, services. If the caller cannot name at least one platform or library, push back: vague Guardians do not survive the pipeline.
...
7. Never reuse a name that already exists in the backlog, the queue, the completed list, or `ai-tools/skills/dungeon-master/SKILL.md`. Check all four before committing.
...
5 to 10 pages of research, glossary-style. Low cognitive load. The Guardian's failure mode is "a curated bookmark list might have been better." Use for productivity glossaries, naming conventions, simple template emitters, README style guides, changelog style guides.

### normal

Roughly 100 pages of research. Daily-driver enforcement, audit, or template work. Bounded decisions: known rule sets, known templates, known tool comparisons. No architecture. Examples: git workflow enforcement, dependency audit, conventional commits, simple onboarding tour libraries, agile process Guardians.

### deep

Thousands of pages of research. Integral architectural role with many variances. The Guardian's wrong decisions create real technical debt or relational damage. Requires canonical platform docs, primary GitHub sources, and 2026-current white papers. Most Tier 1 through Tier 16 platform Guardians live here.
...
If the proposed Guardian feels heavier than `react-guardian`, it is probably `extreme`. If it feels like a curated reference rather than a synthesized framework, it is probably `shallow`.
...
The dominant work is mathematical, statistical, formal, or abstract reasoning. The Guardian's analyst must reason about formulas, distributions, complexity classes, formal grammars, cryptographic primitives, or proofs of correctness.
...
- The CLI is incidental (e.g., a platform Guardian where users will mostly hit the dashboard or SDK, not the CLI).
...
- Gemini 3.1 Pro tops GPQA Diamond and ARC-AGI-2. Apply the override only when the Guardian's analyst will be doing actual mathematical or statistical reasoning, not just touching a numeric domain.
...
3. Cover the Guardian's full scope across the queries. Do not concentrate three queries on one sub-topic and leave the rest unaddressed.
...
1. Read the last `NNN|name` row in `proposed-guardians-queue.md` (the bottom of the file body).
2. Read the highest `### [ ] N. name` heading in `proposed-guardians-backlog.md`.
...
Append a single line to `ai-tools/proposed-guardians-queue.md`, at the very bottom of the body (after the last existing row).
...
- The position number is unique across backlog, queue, completed, and dungeon master.
...
5. Vague domains. "Helpful AI assistant for backend stuff" is not a Guardian. The caller must name a platform, a concern, or a discipline.
6. Guardian names without the `-guardian` suffix.
7. Editing `ai-tools/skills/dungeon-master/SKILL.md` from here. Registration is the final pipeline step, owned by `dm-register`, not by this skill.
