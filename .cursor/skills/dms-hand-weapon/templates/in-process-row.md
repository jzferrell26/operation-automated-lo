...
The exact row format `dms-hand` appends to `ai-tools/proposed-guardians-in-process.md` in Sub-step 2c of `guides/01-pick-and-lock.md`.
...
- `guardian-name` is the kebab-case Guardian name (e.g. `nextjs-guardian`, `auth-guardian`).
...
If a phase failed and `dms-hand` is leaving the row in `in-process` with a marker per the "leave-with-marker" strategy in `guides/10-failure-modes.md`, the row is extended:
...
- `<phase-name>` is the name of the failed phase: one of `command-center`, `loremaster`, `loremaster-auth`, `weapon-forge`, `guardian-creator`, or `dm-registrar`.
...
- The file MAY have YAML frontmatter, but `dms-hand` does not require it. If present, frontmatter fields like `date_updated` and `last_updated_by` are optional metadata.

## When this format applies

`dms-hand` writes this format in:
...
External tools that read `proposed-guardians-in-process.md` (e.g., a future "factory dashboard") can parse the row with a simple split-on-pipe.
