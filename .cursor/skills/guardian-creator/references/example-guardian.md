# Example Guardian: seo-guardian

This file is a reference for guardian-creator showing what a well-formed Guardian looks like end to end. Use it as a model for depth and structure — but every real Guardian should be derived from its own Command Brief and Weapon folder, not copy-pasted.
...
Read `ai-tools/skills/seo-weapon/SKILL.md` first — it is the master index for this Guardian's arsenal.
...
- Every step in the procedure points at a specific file in the Weapon folder, so the Guardian knows exactly where to look for detail.
- Critical directives have reasons attached, so the Guardian can reason about edge cases.
- Escalation is explicit — the Guardian knows what to do when the Weapon doesn't cover the territory.
- The references section is grouped by subfolder and lists every file the Guardian will need.
