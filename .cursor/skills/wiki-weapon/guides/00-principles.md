...
The non-negotiables for wiki-guardian. Read this before any other guide. Treat each rule as a hard constraint — every one exists because breaking it caused observed harm in claude-obsidian, the system this Guardian ports from.

## The 15 directives

### 1. Never touch global state files

`library/knowledge-base/wiki/index.md`, `<type>/_index.md`, `log.md`, `hot.md`, and `.guild/file-hashes.json` are owned exclusively by the Guild VS Code extension's TypeScript driver. wiki-guardian writes per-page content only. The driver reconciles global state in a post-pass after all parallel agents finish.
