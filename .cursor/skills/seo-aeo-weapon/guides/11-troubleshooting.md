...
Hand off to another Guardian when:

- **CSP header changes** → `security-guardian`.
- **Server Actions, advanced RSC patterns** → `react-guardian`.
- **Pages Router → App Router migration** → `react-guardian`.
- **PRD authoring for the remediation plan** → `library-guardian`.
- **Accessibility deep-dive** (WCAG audit) → `ux-ui-guardian`.
