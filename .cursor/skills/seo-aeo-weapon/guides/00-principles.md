...
Read before touching anything else. This guide wraps the canonical playbook's §1 (Introduction & Philosophy) with the `seo-aeo-guardian` Guardian's directives from the Command Brief.
...
6. **Next.js version awareness.** On first contact with a codebase, check `package.json`:
   - `next < 13.4` → App Router not available. Flag and hand off to `react-guardian`.
   ...
   - Pages Router codebases → degraded coverage. Flag and escalate to `react-guardian` for migration.
...
│   └── Deliver phase-by-phase plan; optionally hand off to library-guardian.
└── (Unclear)
...
- Marketing copy, keyword research, editorial work → content Guardian.
...
- Security headers beyond the playbook's defaults → route through `security-guardian`.

---

## Cross-Guardian handoffs

- **`security-guardian`** — CSP headers in `next.config.js > headers()` overlap. Any change adding or relaxing `Content-Security-Policy` routes through `security-guardian` before merge.
- **`react-guardian`** — Pages Router → App Router migration, advanced RSC patterns, Server Actions internals.
- **`library-guardian`** — PRD authoring for phased rollouts when the deploying product wants a feature PRD at `library/requirements/features/feature-<###>-<title>/prd-feature-<###>-<title>.md`.
- **`quality-guardian`** — post-implementation verification; runs last before QA.
