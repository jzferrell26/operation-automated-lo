...
- **security-guardian** — for API key vault strategy, PII audit in prompts, DPA compliance verification, model provider's data-retention policies.
- **mind-guardian** — for cognitive-layer architecture: RAG pipeline design, prompt cascade, three-tier memory, evaluation, coach routing. This Guardian picks the providers; mind-guardian decides how to use them architecturally.
- **devops-guardian** — for Docker container setup for GPU cloud deploys, CI/CD wiring for model inference services, secret injection from environment.
- **library-guardian** — for PRD authorship when a new AI tooling decision needs to be documented as a feature requirement.
