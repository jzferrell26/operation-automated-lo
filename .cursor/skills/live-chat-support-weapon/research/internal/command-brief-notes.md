...
weapon: live-chat-support-weapon
---

# Command Brief Notes: live-chat-support-guardian

## Summary

The `live-chat-support-guardian` is a specialist Guardian for the customer support communication surface. It covers platform selection (Intercom, Crisp, Drift, Plain, Pylon, Help Scout), widget installation, HMAC identity verification, conversation routing, AI deflection, and GDPR data-export discipline. It runs after product decisions but before engineering touches any SDK.
...
- **auth-guardian** owns the identity JWT that feeds the HMAC signing function; live-chat-support-guardian owns what to do with it (consuming it in the `boot()` call).
- **security-guardian** audits the resulting widget config for secret exposure and CSP compliance.
- **db-guardian** owns the production database schema for support data (not in scope here).
...
## Key questions for weapon-forge

- Pylon is Slack-native — its routing model differs from widget-based tools; routing guide needs a Pylon-specific subsection.
- Does loremaster have access to Intercom, Crisp, and Plain developer portals directly? (Yes via Exa.) Prioritize HMAC signing docs and Fin 2.0 capability pages.
