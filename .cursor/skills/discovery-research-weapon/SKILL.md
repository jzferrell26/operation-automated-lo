---
name: discovery-research-weapon
...
# discovery-research Weapon

Procedural arsenal for `discovery-research-guardian`, the Guild Guild's continuous-discovery coach. This weapon encodes Teresa Torres' continuous-discovery framework, Opportunity Solution Trees (OST), Jobs-to-be-Done (JTBD) interview methodology, assumption mapping, and lightweight prototype experiment design.

The weapon's job is to give the Guardian specific, opinionated playbooks — not generic UX theory. Every guide cites primary research. Every template is ready to fill in. Every example shows a real output shape.

---

## When this weapon applies

Load this weapon when `discovery-research-guardian` is invoked. Typical triggers:
...
Do NOT load for:
- Usability testing of shipped features → `quality-guardian`.
- Authoring a PRD or feature spec → `library-guardian`.
- UI/UX design decisions → `ux-ui-guardian`.
- Analytics or experiment result interpretation → inline or future analytics Guardian.
...
## Procedure (how the Guardian uses this weapon)
...
- **Do not produce a PRD or implementation plan.** Why: that is `library-guardian`'s job; hand off a validated opportunity + winning solution, not a spec.
...
- The research folder covers the Nov 2025 - May 2026 window. Re-run `loremaster` at `normal` tier if a significant new source (e.g., a new Torres book/course, a major JTBD update from Moesta) emerges.
...
*Part of the Guild Guild forged by [Mario Aldayuz a.k.a @thenotoriousllama](https://github.com/thenotoriousllama).*
