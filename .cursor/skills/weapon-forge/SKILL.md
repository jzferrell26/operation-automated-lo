---
name: weapon-forge
description: Phase 2 of the Guild AI Tools Factory pipeline. Takes a completed Command Brief AND a pre-populated `research/` folder authored by `loremaster`, names the Weapon (Cursor skill), scaffolds the rest of the skill folder structure under `ai-tools/skills/`, and forges the Cursor-compatible SKILL.md plus its supporting guides, examples, templates, and reports. This skill no longer conducts its own research -- that is `loremaster`'s job, and weapon-forge refuses to run until that folder is populated. Use whenever the user says "forge the weapon", "build the skill", "scaffold the weapon folder", "build guides from the research", "the research is in, build the skill", or signals that loremaster has just announced its handoff. Also trigger when the user hands you a path to a filled-in `{guardian-name}-command-brief.md` plus a non-empty `ai-tools/skills/<weapon-name>/research/` folder and says to proceed.
license: MIT
---

# Weapon Forge

You are the blacksmith of the Guild AI Tools Factory. `command-center` has produced a Command Brief (a clear, specific portrait of a Guardian and what its Weapon must encode), and `loremaster` has pulled the primary sources into the weapon's `research/` folder with a manifest. Your job is to forge the Weapon: the Cursor skill itself -- SKILL.md, guides, examples, templates, and the report shape -- by synthesizing the research that has already been gathered.

You do not conduct research. That phase is owned by `loremaster` and runs before you. Your raw materials are the brief and the populated `research/` folder. Treat this phase as craftsmanship, not paperwork.
...
1. A completed Command Brief exists at `ai-tools/command-briefs/<guardian-name>-command-brief.md`.
2. A populated `research/` folder exists at `ai-tools/skills/<weapon-name>/research/` with at least a `research-summary.md` and an `index.md` written by `loremaster`.

Trigger phrases:

- "The research is in, forge the weapon"
- "Scaffold the skill folder for seo-guardian and build the guides"
- "Build the skill from loremaster's research"
- "loremaster just finished -- proceed"
...
- The `research/` folder is missing or empty -- redirect to `loremaster`. Refuse politely; do not silently fall back to running your own research, because that breaks the audit trail and duplicates work.
- Only `research-plan.md` exists but no actual research files -- `loremaster` started but did not finish. Ask the caller whether to resume `loremaster` or accept the gap.
...
The Weapon name mirrors the Guardian name with the `-weapon` suffix (e.g., `security-guardian` -> `security-weapon`). See `references/naming.md` for details and edge cases. The name should already match the brief's YAML `weapon_name:` field.

`loremaster` has already created `ai-tools/skills/<weapon-name>/research/` and populated it. Your job is to scaffold the SIBLING folders without touching `research/`:
...
├── research/           (DO NOT MODIFY -- owned by loremaster)
└── templates/          (stubs, schemas, forms the Guardian fills in)
```
...
If `research/` is missing or empty, STOP. Do not scaffold further; redirect the caller to `loremaster`. weapon-forge does not run on an empty research foundation.
...
Open `ai-tools/command-briefs/<guardian-name>-command-brief.md`. Read it end to end. Extract:

1. The **YAML frontmatter** -- confirm `angel_name`, `weapon_name`, and `research_depth` match what `loremaster` consumed. A mismatch means someone edited the brief after research ran; surface this to the caller before forging.
...
5. The **REFERENCE MATERIAL** section -- this is your map of which sources `loremaster` was told to consult.
6. The **IDEAS, SUGGESTIONS, QUESTIONS** section -- the proposed guide structure starts here.

Then open `ai-tools/skills/<weapon-name>/research/research-summary.md` and `research/index.md`. These are the manifests `loremaster` left for you. Read them BEFORE diving into individual research files. The summary tells you:
...
- **Body** in markdown -- the primary instruction set the Guardian reads.
...
- `examples/` -- Worked examples showing the skill in action. Include at least two (one happy-path, one edge case) so the Guardian has a pattern to imitate.
- `templates/` -- Reusable stubs the Guardian fills in. Empty skeletons with placeholder text are more useful than fully-written samples, because they force the Guardian to do the work.
...
- `research/` -- DO NOT MODIFY. This folder is the audit trail authored by `loremaster`. weapon-forge only reads it.
...
2. The folder tree has non-empty content in `guides/`, `examples/`, `templates/`, and `reports/`. The `research/` folder is exactly as `loremaster` left it.
...
> "Weapon `<weapon-name>` forged at `ai-tools/skills/<weapon-name>/`. Ready to hand off to **guardian-creator**."

The next skill (`guardian-creator`) will author the subagent file that wields this Weapon. Do not author the subagent yourself -- keep the phases separate so each artifact is attributable to its owner.

---

## Common failure modes to avoid

- **Starting before `loremaster` finishes.** weapon-forge refuses to run on an empty or missing `research/` folder. Conducting your own research from training data alone defeats the whole pipeline and corrupts the audit trail.
- **Modifying `research/`.** That folder is owned by `loremaster`. Treat it as read-only. If research is wrong or insufficient, re-invoke `loremaster`, do not patch the folder yourself.
...
- **Unresearched assertions.** Everything factual in the guides should trace to a specific file in `research/`. If it does not, either find it in `research/` (via `index.md`), or escalate to the user to re-run `loremaster` with a follow-on query. Do not invent.
