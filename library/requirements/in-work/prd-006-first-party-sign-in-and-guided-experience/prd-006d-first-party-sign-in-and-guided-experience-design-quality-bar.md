...
- `Sheet`: the non-modal anchored panel and bottom sheet PRD-006c D6 and D7 specify, sharing focus utilities with a `Dialog` that generalizes the `alertdialog` behaviour in `Button.tsx:180-190,247-257` (focus move, Escape, return) and the drawer's trap in `app-shell.tsx:47-97`. The shell's mobile drawer now renders through `Dialog` itself, at a new `placement="inline-start"`, rather than a hand-built layer that only borrowed `Dialog`'s behaviour (amended 2026-09-20, see Amendments).
...
- Keyboard: every screen operable end to end without a pointer; focus order follows reading order; the focus ring is the shared 2 px ring with 3 px offset and is never obscured (existing assertions extended to the new screens). The walk measures `outline-width: 2px`, `outline-offset: 3px`, and `--focus-color` resolved from the element's own cascade, on the control itself rather than any outline on its parent or its label, walks every focusable element on the screen rather than a fixed stop count, and hit-tests the control's centre and edge midpoints against sticky and fixed layers; one wrapper is a documented, cited exception to "on the control itself" (amended 2026-09-20, see Amendments).
...
- `maxDiffPixelRatio: 0.001`, `animations: "disabled"`, fonts self-hosted (D5) so rendering is deterministic; baselines are generated on the `ubuntu-24.04` runner's Chromium and carry Playwright's platform suffix; Windows and macOS runs compare informationally and never commit baselines. The comparison is narrower than that sentence: it runs only where `CI` is set (that same runner), and every other run skips the comparison rather than failing on rasterisation, with an opt-in flag for a Linux machine that wants to check anyway (amended 2026-09-19, see Amendments).
...
| 006D-AC-012 | Playwright screenshot comparison exists with the D8 configuration, comparing only where `CI` is set (or `OALO_COMPARE_SCREEN_BASELINES=true` asks for it) and otherwise skipping rather than failing on platform rasterisation; baselines exist for every screen, frame, theme, and named state in D3 under `tests/visual/screens/`; the README distinguishes them from the rendering goldens; the suite runs in `pnpm verify:offline` (synthetic screens) and `pnpm test:db` (review screens) and passes. (amended 2026-09-19, see Amendments) | 4 |
...
- **2026-09-19, D8 and 006D-AC-012, where the comparison runs.** Text said: baselines are generated on the `ubuntu-24.04` runner's Chromium, and Windows and macOS runs compare informationally without committing baselines, implying a comparison still happens off that runner. Code does: `playwright.config.ts` gates the comparison on `Boolean(process.env["CI"]) || process.env["OALO_COMPARE_SCREEN_BASELINES"] === "true"`; `.github/workflows/screen-baselines.yml` regenerates baselines on `ubuntu-24.04` and hands them back as two artifacts for a person to review and commit, nothing in the workflow commits on its own; `OALO_SCREEN_SNAPSHOT_DIR` points both projects' captures at a directory outside the repository, which is how D9's sign-off captures a full set of every screen on any machine without touching a baseline; and `tests/visual/screens/README.md` states the platform rule this way. Why: text rasterises differently on every platform, so a comparison against a baseline drawn on a different platform would fail on rasterisation rather than on a real visual regression, which is not a useful signal; skipping the comparison off the runner (with an explicit opt-in for a Linux machine that carries the runner's fonts) is the narrower and more honest rule than "compare informationally." Decision recorded by the implementing lane at commit `fb4c588`. Touches: 006D-AC-012.
- **2026-09-20, D4, the shell's mobile drawer moves onto `Dialog` itself.** Text said: `Dialog` "generalizes ... the drawer's trap in `app-shell.tsx:47-97`," describing the drawer as a separate hand-built implementation that the new primitive would model itself on. Code does: `app-shell.tsx`'s drawer now renders `Dialog` directly at a new `placement="inline-start"`; the hand-built trap, Escape handler, scroll lock, and focus return are deleted, and the wrapper that remains carries only the media gate that keeps the layer closed above the mobile frame. Why (from `ux-ui-guardian`'s review, finding F-24): a drawer that only borrowed `Dialog`'s behaviour and a drawer built from `Dialog` are two implementations of one contract, which is exactly how the two had already begun to drift (the trap's own `resolveTabTarget` differed from the primitive's). Test: `tooling/tests/unit/design-quality/hand-built-layers.test.ts` reads the ARIA spelling and the scroll lock rather than the tag; `packages/ui/src/components/overlay-and-feedback.test.tsx` asserts the placement on scrim and panel; `app-shell.integration.test.tsx`'s trap, scroll lock, Escape, and focus-return case passes unchanged through the primitive. Decision recorded by the implementing lane at commit `61cff02`. Touches: 006D-AC-003.
- **2026-09-20, D3, the route error, loading, and unverified-email surfaces gain a screen to be reviewed from.** Text said these three are screens in D3's scope, without naming how a reviewer or the screenshot suite reaches them. Code does: `apps/web/src/app/(authenticated)/design-surfaces/page.tsx`, gated on `canRenderSyntheticDemo()`, renders all three inside the real shell from placeholder values; the page returns HTTP 200 rather than 404 because the shell's layout streams before the page itself calls `notFound()`, which the page and its test state as a fact about the framework rather than asserting a property of it. Why (from `ux-ui-guardian`'s review, findings F-27 and F-28): these three were named in D3 and the rubric and were in no review and no suite, because nothing rendered them outside the real failure or unverified-account condition that a review run cannot manufacture on demand. Test: the surface is a screen in `SYNTHETIC_SCREENS` with axe, keyboard, motion, target size, and a screenshot at four frames in both themes, its three surfaces asserted by name; an assertion from the review side confirms the not-found page is what a real unknown route returns and that none of the three design surfaces is reachable there. Decision recorded by the implementing lane at commit `61cff02`. Touches: 006D-AC-007, 006D-AC-008.
- **2026-09-20, D7 and 006D-AC-009, precision of the ring measurement.** Text said the focus ring assertions are "existing assertions extended to the new screens," without stating what those assertions measured. Code does: `drawsRing` measured `outline-width: 2px` and `outline-offset: 3px` on the control, its parent, or its label, whichever carried it, and the walk capped at 24 stops; nothing asserted SC 2.4.11 (that the indicator is not entirely hidden). It now measures all three properties, including `--focus-color`, on the control itself, with one wrapper named and cited as the sole exception; budgets its stops from the page's own focusable-element count rather than a fixed cap; and hit-tests the control's centre and edge midpoints against sticky and fixed layers. Why (from `ux-ui-guardian`'s review, finding F-29): a ring on a parent or a label is not evidence the control itself is indicated, and a capped walk silently stops reviewing a screen with more than 24 focusable elements. The real defect the tightened walk found: the support region's summary carried Chromium's default `outline: auto`, not a product ring; fixed in `primitives.css` on `[data-support-details] > summary`. Decision recorded by the implementing lane at commit `61cff02`. Touches: 006D-AC-009.
- **2026-09-20, checked, no divergence found: D-001 and the ring's color.** Wave 7o's ruling (`design-system-guardian`, commit `0b9c3fb`) makes `--focus-color` a literal per theme (Light `#2f6fed`, Dark `#8bb0ff`) rather than the tenant accent, because the ring is an accessibility affordance and not a brand surface; the oalo-teal tenant's ring now changes visibly in both themes, and `token-contrast.unit.test.ts` pins the ring against every surface it can land on. Neither D4 nor D5 makes a claim about what token or color the ring draws from, so there is no sentence here to amend; the ruling is recorded in `library/knowledge/private/ux-ui/06-review-rubric.md` section 5 and the token files themselves.
- **2026-09-20, checked, no divergence found: the baseline-note guard's shallow-checkout behaviour.** Wave 7o (commit `ead116a`) made `changedFiles()` in `tooling/tests/unit/design-quality/baseline-note.test.ts` throw the real git error when `GITHUB_BASE_REF` is set and the base cannot be resolved, instead of swallowing it into an empty list that let the 006D-AC-013 guard pass silently, and gave the `verify` job's checkout `fetch-depth: 0` in `ci.yml` so `origin/${GITHUB_BASE_REF}` is present to diff against. Neither D8 nor 006D-AC-013's wording describes the guard's mechanism (a git-based diff against a base ref) or its failure behaviour on a shallow checkout, so there is no sentence here that now reads wrongly. This also settles the open question at the foot of this document about whether the check is a unit test over the pull request body or a CI step using `gh pr view`: it is neither, it is a unit test that diffs against the base ref directly.
- **2026-09-21, D7 and 006D-AC-009, the sheet's sticky footer gets scroll padding.** Text said (`library/knowledge/private/ux-ui/03-components/sheet-and-dialog.md`, before this entry): the sheet's scrollable body caps at `min(32rem, 70vh)` and its sticky footer never scrolls a panel's own Continue control out of sight, with no statement about a field the footer could cover instead. Code does (Wave 7t, commit `3e0b3bb`): `packages/ui/src/components/overlay.module.css`'s sheet body gains `scroll-padding-block-end: calc(var(--target-min-size) * 2 + var(--space-3))`, and `tests/browser/review/guided-setup.accessibility.spec.ts` gains `assertThePanelsControlsCarryTheRing`, which asserts the D7 ring (width, offset, and `--focus-color`) on the walkthrough panel's own controls (its Continue/"Let's go"/Done slot, its "Not now", and its close control) at every checkpoint of every cell, not only on the D3 screens the walk already covered. The defect the new measurement found: without the scroll padding, tabbing to a control past the end of the scroll box (guided setup step 2's NMLS field, measured at Light 1180x900) scrolled it to the box's end, which sat entirely under the sticky footer, ring and all, a WCAG 2.2 SC 2.4.11 failure the existing walk could not see because the walkthrough panel is not itself a D3 screen and is in no screen's tab order. Why: the panel's primary control carries three different labels across the walkthrough ("Let's go", "Continue", "Done"), so a single checkpoint could not have measured all three; running the assertion at every checkpoint measures each. `tests/browser/review/review-change-password-saved.spec.ts` also gained `expectKeyboardReachesEveryControl` on the change-password screen's saved state (previously only its default state was walked), in its own loop after the screenshots so the ring the walk leaves never lands in a captured baseline. Touches: 006D-AC-009 (coverage widens; its own wording, "every D3 screen," still holds, since the panel and the saved state are additional coverage beyond what the row requires, not a replacement for it).
