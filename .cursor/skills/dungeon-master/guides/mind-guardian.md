...
- Explicit handoff lines for any finding belonging to another Guardian (db-guardian, react-guardian, security-guardian, library-guardian, quality-guardian).
...
mind-guardian is a **highest-priority Guardian** in the implementation loop because every AI feature touches it. Step in the extended loop: **Plan → Implement → mind-guardian review → security-guardian → db-guardian → quality-guardian**. mind-guardian runs early (before security and db) because architectural / cognitive-stack drift changes the surface area downstream Guardians audit.
