...
2. Run `pnpm guild-sync --status` to confirm the wiki is current.
3. Tell the user: what was created/migrated, that notes/ was not touched, next steps for creating content.

## Error handling

- **Script not found**: run `pnpm install` from guild-suite root first.
...
- **Not in a recognized repo**: make sure --repository matches one of the ALL_REPOS list in the script (`guild-suite`, `guild-marketing`, `guild-secure`, `guild-code`, `guild-website`, `guild-cloud`, `guild-shim`, `guild-harness`, `guild-shared`).
