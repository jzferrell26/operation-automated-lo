...
This shapes `templates/qa-report.md`. Adopt the industry norm of severity-tagged findings with file:line coordinates, but add the plan-traceability table and the five-axis scorecard — those are the Guardian's unique contribution. Also confirms the Command Brief's decision to produce a markdown report (not JSON, not a PR comment thread): markdown is the lingua franca of these tools and of GitHub/Cursor UI.
