...
Once the seven-artifact folder is populated, this Guardian's job is done.
The companion Guardian — `ux-ui-guardian` — takes ownership and enforces
...
The Guardians are paired but strictly separated. After bootstrap, the
...
`README.md` names the owning Guardian explicitly:
...
Both Guardians inherit:
`<guardian-name>: <section>: <change>`
...
### 4. Optional — stub the companion Guardian
...
Leave the stub at "draft" status; `guardian-creator` finishes it in Phase
3 of the Legendary Guardian Factory pipeline.

## What this Guardian does NOT do after handoff
...
## Triggering this Guardian again (rare)
