...
This file summarizes what guardian-creator needs to produce a valid, high-quality Cursor subagent. When in doubt, fetch the live documentation — Cursor's subagent system is evolving. Use `WebFetch` on the URL above or search `web_search_exa` for "cursor subagents documentation" to confirm current requirements.
...
2. **Paired Weapon** — link to the Weapon skill folder.
...
6. **References to skill files** — the full list of Weapon files the subagent should read.
...
Proactive subagents are invoked automatically when the orchestrator sees their domain in the task. On-demand subagents require an explicit mention (e.g., "use the security-guardian to review this").

Defaults by Guardian type:

- Review/audit Guardians (read-only, produces reports): proactive.
- Generation Guardians (writes new files): proactive, but with a confirmation step in the procedure.
- State-mutating Guardians (deploys, deletes, commits): on-demand.
...
seo-guardian is a technical-SEO reviewer invoked on pull requests. It reads code diffs, flags regressions in crawlability and indexability, and produces a severity-ranked report.

## Paired Weapon
`ai-tools/skills/seo-weapon/` — read its SKILL.md first.

## Procedure
1. Read the PR diff. Identify SEO-relevant files per `seo-weapon/guides/01-file-classification.md`.
2. For each file, apply the matching checklist from `seo-weapon/guides/02-checklists/`.
3. Classify each finding per `seo-weapon/guides/03-severity.md`.
4. Produce a report using `seo-weapon/templates/report.md`.
...
Utilize the Read tool to understand your skills at `ai-tools/skills/seo-weapon/` including:
...
- **No references**: Guardian doesn't name its Weapon files — will not open them reliably.
- **Overlapping scope**: Two Guardians with nearly-identical descriptions compete for the same work.
- **Missing escalation**: Guardian does not say what to do when it's unsure, so it guesses.
