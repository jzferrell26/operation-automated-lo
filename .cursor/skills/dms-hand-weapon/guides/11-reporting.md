...
The final report is `dms-hand`'s only output to the caller. The tracking-file mutations and on-disk artifacts are the durable products; the report is the human-readable receipt. Make it concise, complete, and structured so an orchestrator (or human) can decide what to do next.
...
Open with a single line stating what happened, in past tense, with the position number and guardian name. Examples:

- `Forged Guardian 001|nextjs-guardian. Pipeline complete.`
- `Queue empty. No work to do.`
- `Cycle stopped at Phase 2 (weapon-forge). Row 005|vite-guardian remains in-process with marker.`
...
| File | Before | After |
|---|---|---|
| `proposed-guardians-queue.md` | Top row was `NNN\|guardian-name` | Row removed; new top row is `MMM\|next-guardian-name` (or "queue empty") |
| `proposed-guardians-in-process.md` | Empty | Row was appended then removed; file is now empty |
| `proposed-guardians-completed.md` | Empty / N rows | Row appended; file now has N+1 rows |
...
- Dungeon Master's roster row: added to `ai-tools/skills/dungeon-master/SKILL.md` (with line number if possible)
...
| 2 | weapon-forge | 0:08:11 | OK |
| 3 | guardian-creator | 0:01:02 | OK |
| 4 | dm-registrar | 0:00:18 | OK |
...
- Other items that warrant human follow-up but do not invalidate the Guardian.
...
- `Cycle complete. Invoke dms-hand again to process the next queue entry, or stop here.`
- `Cycle paused at Phase 2. Resolve the failure (see in-process row marker) and re-invoke.`
- `Queue empty. No further dms-hand invocations needed until session-zero queues more rows.`

This section is the EXPLICIT statement of `dms-hand`'s stop rule. By writing the next-steps line, the Guardian makes clear that it does not auto-advance. The orchestrator decides.
...
This is the signal to the orchestrator that the run is over. Do NOT continue with chat after this line. Do NOT recursively invoke `dms-hand`. Do NOT pick up the next queue entry.

This line is load-bearing per `guides/00-principles.md` non-negotiable #7 ("Stop after dm-registrar"). Without it, the orchestrator might assume `dms-hand` is still working and wait indefinitely.

## A note on context compaction

If `dms-hand` is invoked for a deep-tier cycle that takes hours, context compaction may erase the early steps of the run before close-out. The final report becomes hard to assemble.
