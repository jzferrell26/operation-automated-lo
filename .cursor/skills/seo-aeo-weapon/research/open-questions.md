...
**Brief:** "Should the Weapon include a 'migration from Pages Router' guide, or is that out of scope? The source doc assumes App Router throughout."

**Default in this forge:** Out of scope. `guides/00-principles.md` declares Next.js 14+ App Router scope and instructs the Guardian to flag Pages Router codebases as degraded coverage, escalating to `react-guardian` for the App Router migration. Preserves the canonical playbook's scope.
...
**Default in this forge:** Always authored (playbook §9 is in scope), conditionally *applied* by the Guardian. The Guardian's Step 1 scoping determines whether the target is a local-business site; if not, `guides/08-local-seo.md` becomes low-priority reading rather than checklist-blocking.
...
**Default in this forge:** Documented in `research/refresh-cadence.md`. Owner left unspecified; `seo-aeo-guardian` Guardian can execute the review when prompted ("refresh seo-aeo research"). The Guardian re-runs the research plan's queries and updates dated notes.
