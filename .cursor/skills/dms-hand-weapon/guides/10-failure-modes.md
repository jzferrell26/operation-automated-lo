...
The catalog of "what to do if X" for every failure mode `dms-hand` might encounter. Each entry has a trigger, a diagnosis, and a recovery procedure.

The earlier in the cycle `dms-hand` recognizes a failure, the cheaper the recovery. Use this guide as a checklist whenever a step does not produce its expected output.

## Pre-flight failures (before Step 1)

### F0.1: Queue is empty

- **Trigger:** `ai-tools/proposed-guardians-queue.md` body has no rows below the YAML frontmatter.
- **Diagnosis:** All proposed Guardians are either in-flight or completed. Nothing to do.
...
- **Trigger:** `ai-tools/proposed-guardians-in-process.md` has one or more non-blank lines on entry.
- **Diagnosis:** A prior cycle crashed mid-run or did not complete close-out. The row visible in in-process tells you which Guardian was being processed and (if it has a `|failed:<phase>` marker) which phase failed.
- **Recovery:** STOP this cycle. Do NOT pick a new queue row. Surface the orphaned row to the caller and route to `examples/recovery-from-crashed-prior-run.md`. The human reviewer decides whether to:
  - Re-invoke `dms-hand` to retry from the failed phase (manual, not automatic).
...
- **Recovery:** Flag in the final report. The discrepancy does not block this cycle's work because `dms-hand` picks rows from the body directly, not from the frontmatter count. However, a future cycle reading the frontmatter for guards could be misled. Recommend the user (or `session-zero`'s owner) reconcile the count.

## Pick-and-lock failures (Step 1-2)

### F1.1: Queue row text contains unexpected characters

- **Trigger:** A row line does not match the `NNN|guardian-name` format (e.g., extra fields, lowercase failures, missing pipe).
- **Diagnosis:** Probably a manual edit gone wrong, or `session-zero` produced a malformed row.
- **Recovery:** STOP. Surface for the caller to fix the row. Do not attempt to repair the row automatically; the position number and guardian name are load-bearing identifiers.

### F1.2: Two `dms-hand` invocations race on the same queue row

- **Trigger:** During Sub-step 2a (delete row from queue), the StrReplace fails because the row text is not present.
- **Diagnosis:** A sibling `dms-hand` invocation already deleted the same row.
- **Recovery:** The first-to-delete wins. The second invocation should:
  1. Re-read the queue. The top row may now be a different row.
  2. Check `in-process.md` for two duplicate rows (this is the race signature). If two rows of the same guardian-name are present, manual cleanup is needed: delete the duplicate, leave the original to proceed normally.
  3. If `in-process.md` has only one row of the same guardian-name AND it was just appended by the OTHER invocation, this invocation should STOP per F0.2 (in-process non-empty refusal).

### F1.3: Append to in-process fails

- **Trigger:** Sub-step 2c fails to write the row to `ai-tools/proposed-guardians-in-process.md`.
...
- **Trigger:** Searching `ai-tools/proposed-guardians-backlog.md` for `### [ ] N. <guardian-name>` (or `### [x] N. <guardian-name>`) returns zero matches.
- **Diagnosis:** Queue and backlog are out of sync. The queue row references a Guardian that does not exist in the backlog.
...
- **Diagnosis:** The Guardian was already completed but the queue still had its row. Another desync.
- **Recovery:** STOP. Delete the in-process row (do NOT roll back to queue; the queue row was the wrong one). Surface for the caller. The completed Guardian's artifacts should already exist on disk; no further forging is needed.
...
- **Diagnosis:** The phase has over-stepped. Examples: `weapon-forge` modifies `research/`, `guardian-creator` modifies the Weapon folder, `dm-registrar` modifies the Guardian file.
- **Recovery:** STOP. Surface as a contract violation. The Guardian may still be usable, but the audit trail is corrupted. Recommend either reverting the cross-phase modifications or starting the cycle over.

### F3.3: Lore-historian Firecrawl or Exa auth error

- **Trigger:** Lore-historian's final message includes an auth-failure surface.
- **Diagnosis:** External research tools are not authenticated.
- **Recovery:** STOP. Surface to the caller; they need to run `firecrawl login --browser` or `/exa-setup` and re-invoke the cycle. The in-process row stays in place; mark with `|failed:loremaster-auth|YYYY-MM-DD`.

### F3.4: Lore-historian depth tier missing

- **Trigger:** Lore-historian's pre-flight check reports that the depth tier is missing from both the Command Brief's frontmatter and the backlog entry.
...
- **Recovery:** STOP. Manually patch the Command Brief's frontmatter with the depth tier from the backlog. Re-invoke loremaster.
...
- **Trigger:** Step 10d's StrReplace cannot find the `### [ ] N. <guardian-name>` heading.
