...
relevance: critical
topic: cors
weapon: http-rest-fundamentals-weapon
---
...
- `guides/04-cors.md`: this spec is the definitive source for the complete preflight flow, the wildcard-with-credentials prohibition, and the Expose-Headers behavior. Every CORS rule the weapon encodes must cite the Fetch spec.
- The "wildcard + credentials = Critical" directive in the guardian's identity matches the spec exactly — weapon-forge should encode this as a non-negotiable Critical-severity finding.
...
- Contradiction note: some developer blog posts simplify CORS incorrectly (e.g., claiming server-side CORS protects APIs from non-browser clients). Weapon-forge should include a "what CORS does NOT do" clarifying note.
