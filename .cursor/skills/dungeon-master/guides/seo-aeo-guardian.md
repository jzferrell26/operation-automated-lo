...
Do **not** invoke for non-Next.js stacks (Nuxt, SvelteKit, Astro, plain HTML) — the Guardian can extract framework-agnostic principles but will decline to claim fidelity.

Do **not** invoke for marketing copy or keyword research — that is a content Guardian's domain.
...
- The Next.js version (the Guardian will verify on first contact — the Metadata API and `viewport` export split vary by version).
...
- Invoked on a Pages Router project without the degraded-coverage flag — the Guardian will flag and hand App Router migration to `react-guardian`.
- Invoked on a non-Next.js stack — the Guardian will decline full fidelity and offer framework-agnostic principles only.
- Invoked to write marketing copy — route to a content Guardian; this Guardian handles structure, not words.
- Invoked alongside CSP or security-header changes — the Guardian surfaces the header changes and routes them through `security-guardian` before merge.
