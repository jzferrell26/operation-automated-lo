# Command Brief Summary: technical-writing-craft-guardian

**Source brief:** `ai-tools/command-briefs/technical-writing-craft-guardian-command-brief.md`
**Created:** 2026-05-20
**Purpose:** Cross-reference for weapon-forge when building guides and templates.

---

## Guardian identity

`technical-writing-craft-guardian` is the Guild Guild's documentation craft specialist. It owns the *writing itself*, not the platform that hosts the docs. Its peer boundaries are:

| Guardian | What they own | Handoff signal |
|---|---|---|
| docs-site-guardian | Platform selection, deployment, search config | "which tool?" or "how do I deploy?" |
| library-guardian | Folder structure, PRD authorship, knowledge-base org | Decides *where* a doc lives |
| api-docs-guardian | OpenAPI spec enrichment, SDK generation | Machine-readable spec content |
| seo-aeo-guardian | Metadata, schema markup, AEO optimization | Discoverability |
| readme-writing-guardian | README files only (specialized subset) | README-specific narrow surface |

---

## What the Weapon must encode

The brief specifies eight action steps the Guardian must execute. The Weapon's guides must give the Guardian the knowledge to perform each:
...
| File | Purpose |
|---|---|
| templates/scorecard.md | Blank scorecard for the Guardian to populate |
| templates/ghostwrite-brief.md | Intake form for ghostwriting mode |
| templates/code-example-checklist.md | Runnable checklist the Guardian applies to every code block |

---

## Open questions from the brief (for weapon-forge)

1. Does Vale have a Diataxis-specific ruleset in 2026 that weapon-forge should reference?
...
## Critical directives for the Guardian (from brief)
...
- **Respect the supplied style guide; do not impose Guild defaults when a house style is provided.**
