---
name: blogging-content-strategy-weapon
...
- Technical SEO implementation (schema markup, robots.txt, Core Web Vitals, hreflang) → `seo-aeo-guardian`
- CMS setup or hosting → `website-guardian`
- Social media amplification or paid distribution
- Individual post drafting (though this Weapon produces detailed post briefs that a human or AI writer follows)
...
| Guide | Use when |
|---|---|
| `guides/00-principles.md` | Reviewing the non-negotiables that govern all advice from this Weapon |
...
- **Hand off technical SEO to `seo-aeo-guardian`.** This Weapon owns strategy and copy; schema, sitemap, and Core Web Vitals are out of scope.

---

## Handoff

When this Weapon's output will be consumed by another Guardian:

- **Cluster map** → hand to `seo-aeo-guardian` for schema markup and technical implementation of the pillar page architecture.
...
*Weapon forged by `weapon-forge` from `blogging-content-strategy-guardian-command-brief.md` and research gathered by `loremaster`. Part of the Guild AI Tools Factory by [Mario Aldayuz a.k.a @thenotoriousllama](https://github.com/thenotoriousllama).*
