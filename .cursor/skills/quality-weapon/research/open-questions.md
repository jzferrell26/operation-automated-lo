...
**Forge-time decision:** Shipped as a standalone Python 3 script with only stdlib dependencies (no `pip install` needed). If the host dev environment does not have Python 3 readily available (e.g., pure Windows-without-Python setup), the Guardian can still do the extraction manually — the script is an accelerator, not a dependency.
...
## 5. Cross-Guardian boundary with `library-guardian`
...
**Forge-time decision:** Yes — `guides/00-principles.md` documents the cross-Guardian handoff explicitly, including:
...
If you want the Guardian to proactively ping `library-guardian` when it finds a plan defect (vs. just flagging it in the report), I'll add an escalation path.
