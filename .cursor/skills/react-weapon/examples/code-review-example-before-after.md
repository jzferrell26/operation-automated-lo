...
**`user-search.tsx:23`** — `<input>` has no associated label. Add `<label>Search users <input ... /></label>` with proper role. (Citation: `guides/06-forms.md §accessibility`.) Hand off to `ux-ui-guardian` if deeper a11y review is wanted.
...
**Cross-Guardian handoff:** `ux-ui-guardian` for a11y deep-dive after the label fix lands.
