...
Cursor does not mandate these names, but ai-tools uses them consistently so Guardians know where to look.
...
2. **SKILL.md body** — loaded when the skill triggers. Should teach the Guardian the shape of the work and point at details.
3. **Supporting files** — read on demand via the Guardian's `Read` tool calls, following pointers in SKILL.md.
