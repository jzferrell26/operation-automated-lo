...
The operational core of `dms-hand`. This guide covers Steps 1 and 2 of the Command Brief's ACTION list: picking the top queue row and locking it into `in-process` BEFORE doing any phase work.

## Step 1: Pick the top row from the queue

Open `ai-tools/proposed-guardians-queue.md`. The file has YAML frontmatter (lines 1-40 roughly) followed by a body of rows. Each row has the exact form:

```
NNN|guardian-name
```

where `NNN` is a zero-padded 3-digit position number and `guardian-name` is the kebab-case Guardian name.

The top row is the lowest `NNN` still present. The queue is monotonic by `pickup_protocol`'s contract: positions are never reused and never inserted in the middle. `session-zero` only ever appends to the bottom.

Capture three things from the picked row:

1. `NNN` -- the position number (preserve the zero-padding).
2. `guardian-name` -- the Guardian name as it appears in the row.
3. The full row text `NNN|guardian-name` -- this is what you will move.
...
### Sub-step 2a: Delete the picked row from `proposed-guardians-queue.md`

Edit the queue file to remove the EXACT row text `NNN|guardian-name`. Do not delete the surrounding rows. Do not renumber. Do not reorder.
...
- Set `last_updated_by:` to `dms-hand`.
...
### Sub-step 2c: Append the row to `proposed-guardians-in-process.md`

Append a single line to `ai-tools/proposed-guardians-in-process.md`:
...
The research surfaced a stale section in `ai-tools/proposed-guardians-queue.md`'s YAML frontmatter. The `pickup_protocol` text in the queue file describes a ONE-stage lifecycle (queue -> completed directly). The Command Brief and this guide specify a TWO-stage lifecycle (queue -> in-process -> completed).

`dms-hand` follows the TWO-stage lifecycle. Treat the queue file's `pickup_protocol` text as out-of-date documentation that needs a manual edit (suggested wording: replace "appends the row to `proposed-guardians-completed.md` with a status marker and ISO date" with "appends the row to `proposed-guardians-in-process.md`; the row moves to `proposed-guardians-completed.md` only after all five phases succeed via `dms-hand`'s close-out step").
