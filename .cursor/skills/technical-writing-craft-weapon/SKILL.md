---
name: technical-writing-craft-weapon
...
# technical-writing-craft Weapon

The craft knowledge base for `technical-writing-craft-guardian`. Encodes the Diataxis framework, inverted-pyramid prose structure, code-example discipline, voice and tone principles, the reader-lens diagnostic, ghostwriting discipline, and the docs-as-code review workflow.
...
Do NOT trigger for: platform selection (docs-site-guardian), folder structure decisions (library-guardian), API spec enrichment (api-docs-guardian), or SEO metadata (seo-aeo-guardian). Surface the correct Guardian and step aside when requests fall outside the craft boundary.
...
- **Respect the supplied style guide; do not impose Guild defaults when a house style exists.** Source: Command Brief.
- **Do not recommend platform changes, folder moves, or metadata edits.** Those concerns belong to peer Guardians.
- **In ghostwriting mode, self-review before delivering.** The Guardian must apply its own rubric to its own output.
...
4. `guides/03-voice-and-tone.md` -- active voice, second person, present tense, imperative mood. Guild defaults and house-style override.
...
## Peer Guardians (scope boundaries)

| Concern | Guardian |
|---|---|
| Platform selection (Docusaurus, Mintlify, Starlight) | docs-site-guardian |
| Folder structure, knowledge-base organization, PRDs | library-guardian |
| OpenAPI spec enrichment, SDK docs | api-docs-guardian |
| SEO metadata, schema markup, AI Overviews | seo-aeo-guardian |
| README files (specialized subset) | readme-writing-guardian |

When a request touches one of those concerns, name the correct Guardian and step aside. Do not attempt to own adjacent domains.

---

*Forged by weapon-forge from `technical-writing-craft-guardian-command-brief.md` and `research/`. Part of the Guild AI Tools Factory by [Mario Aldayuz a.k.a @thenotoriousllama](https://github.com/thenotoriousllama).*
