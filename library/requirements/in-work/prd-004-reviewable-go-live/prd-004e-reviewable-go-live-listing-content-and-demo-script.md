...
| ID | Criterion | Status |
|---|---|---|
| 004E-AC-001 | A customer-facing scope statement exists and describes only install → setup → create → persist → approve. | DONE ([what-is-automated-lo](../../../knowledge/public/overview/what-is-automated-lo.md)) |
| 004E-AC-002 | A customer-facing FAQ exists and contains no Meta publish, lead delivery, billing, or performance claim. | DONE ([open-house-boost-faq](../../../knowledge/public/faqs/open-house-boost-faq.md)) |
...
| 004E-AC-007 | Every customer-facing claim is traceable to a criterion or merged PR in the claim audit table. | DONE (copy pack, claim audit section) |
...
`004E-AC-001` through `004E-AC-008` are marked `DONE` because the artifacts exist in this repository and can be read. They are not marked `VERIFIED`: an independent reviewer has not yet audited the copy against the demonstrated scope, and `quality-guardian` owns that verdict.
