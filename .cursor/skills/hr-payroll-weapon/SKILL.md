...
Procedural arsenal for `hr-payroll-guardian`, the Guild AI Guild's HR infrastructure and payroll decision specialist. This Weapon encodes the five core decision moments a software startup encounters in its people-ops lifecycle, grounded in 2026-current research.
...
4. **Hold the legal-advice fence.** The Guardian provides decision frameworks and flags risk; it does not provide legal opinions. "Consult an employment attorney" and "consult a CPA" are mandatory at AB5/DOL analysis branch points and at any tax-consequence decision.

---

## Research foundation

This Weapon was built from 13 external source notes in `research/external/` covering all major platform comparison surfaces, EOR pricing intelligence, worker classification law, and 2026 regulatory changes. Key data points:
...
Open questions from research (weapon-forge flagged; verify before recommending):
...
Load this weapon when `hr-payroll-guardian` is invoked. Typical triggers:
...
Do NOT load for:
- Performance management, OKRs, 1:1s (no peer Guardian covers this yet; defer to library-guardian)
- Recruiting, ATS platforms, offer letters (future talent-guardian)
- Immigration / work visa strategy (flag to consult immigration attorney)
- Accounting software selection beyond payroll integration (future finance-guardian)
- SSO/identity provisioning for the payroll platform (auth-guardian)
- HR data schema design for custom tables (db-guardian)
- Contractor invoice payment flows (payments-guardian)
...
+- research/
   +- research-plan.md                   (loremaster: queries, budget, time window)
...
| Situation | Route to |
|---|---|
| SSO/SCIM provisioning for payroll platform | `auth-guardian` |
| HR database schema design (custom tables) | `db-guardian` |
| PRD authorship for people-ops features | `library-guardian` |
| SSN/PII exposure in payroll API integrations | `security-guardian` |
| Contractor invoice payment and AP flows | `payments-guardian` |
| Company formation before setting up payroll | `incorporation-startup-stack-guardian` |
| Immigration / visa strategy | "Consult an immigration attorney" (outside Guild scope) |
...
- **Regulatory content:** re-run `loremaster` at shallow tier after any major IRS, DOL, or NLRB rulemaking; after each California AB5 amendment; or when a new PFML state program launches.
...
*Forged by `weapon-forge` from `hr-payroll-guardian-command-brief.md` and `research/`. Part of the Guild AI Tools Factory by [Mario Aldayuz a.k.a @thenotoriousllama](https://github.com/thenotoriousllama).*
