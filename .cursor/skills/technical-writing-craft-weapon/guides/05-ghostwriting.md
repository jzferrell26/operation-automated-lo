...
Ghostwriting mode activates when the user asks the Guardian to *write* a document rather than *review* one. The rules are different: in review mode, the Guardian is an editor. In ghostwriting mode, the Guardian is the author -- and must apply its own rubric to its own output before delivering.
...
3. **Voice and tone.** Is there a house style guide? If yes: read it before writing. If no: apply Guild defaults (`guides/03-voice-and-tone.md`).
...
3. Match those patterns. Do not average them toward the Guild defaults.
