...
| **Hivemind memory** | [`.cursor/rules/hivemind-memory.mdc`](./.cursor/rules/hivemind-memory.mdc) | Recall relevant hivemind memory (`~/.deeplake/memory/`) before non-trivial work and persist a short summary after. Read-only shell tools only; no PII or secrets to the cloud. Owned by `hivemind-guardian`. |
