...
# Command Brief Analysis: markdown-mdx-content-pipeline-guardian

## Guardian Identity Summary

`markdown-mdx-content-pipeline-guardian` owns everything between a raw `.md` or `.mdx` source file and its final HTML/JSX/React output. It does NOT own: documentation platform selection (`docs-site-guardian`), React component architecture (`react-guardian`), broader security audits (`security-guardian`), or SEO/AEO concerns (`seo-aeo-guardian`).

## Six Processing Layers

The weapon must encode playbooks for these six layers, each as its own guide:
...
5. **Route platform-selection questions to docs-site-guardian** (pipeline ≠ platform)
...
## Weapon Guide Structure (from Brief)
...
- `docs-site-guardian` → owns platform selection (Starlight, Docusaurus, Mintlify) and may recommend expressive-code for Starlight
- `react-guardian` → owns `mdx-components.tsx` component map wiring
- `security-guardian` → owns broader XSS audit beyond what this Guardian configures
