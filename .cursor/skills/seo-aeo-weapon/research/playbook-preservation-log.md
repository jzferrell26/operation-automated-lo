...
| §2.1 | next.config.js | Verbatim → `templates/next.config.js` | All code preserved. Added inline comment cross-referencing `security-guardian` for the security-headers block. |
...
- **`guides/00-principles.md`** — new guide wrapping the brief's SUBAGENT CRITICAL DIRECTIVES around the playbook's Introduction. Not a playbook section; necessary for the Cursor skill pattern (principles file is a Guild convention — see `quality-weapon`, `security-weapon`).
...
- **`examples/audit-ecommerce-site.md`, `implementation-blog-post.md`, `core-web-vitals-remediation.md`** — worked examples required by weapon-forge's done-checklist. Not in the playbook; Cursor-skill convention.
...
- **Cross-Guardian references** — `security-guardian` (CSP overlap), `react-guardian` (App Router), `library-guardian` (PRD authoring) — annotated where relevant. Not in the playbook; required by the brief.
