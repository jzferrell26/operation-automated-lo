...
## Summary
Canonical Cursor documentation for subagents. Defines subagent fundamentals (specialized AI assistants Cursor's agent can delegate tasks to), foreground vs background mode, the orchestrator pattern (which is exactly what `dms-hand` implements), and Cursor 2.5+ nested subagent support. The orchestrator pattern section is direct prior art: a parent agent coordinates planner / implementer / verifier subagents in sequence with structured handoffs.
...
- `guides/00-principles.md` should cite the orchestrator pattern verbatim. `dms-hand` is the canonical Cursor orchestrator: it does NOT do domain work itself, it dispatches the planner/implementer/verifier (in our case: command-center / weapon-forge / quality-via-the-Guardian-build-out).
- `guides/05-phase-15-loremaster.md` should explicitly use "foreground mode" framing for the loremaster dispatch. The phase BLOCKS until research is complete; that is foreground semantics.
- The skill-vs-subagent table justifies why `loremaster` is a subagent (context-heavy, multi-step, isolated context window) while the other four phases are skills (single-purpose, one-shot per phase). `guides/00-principles.md` should restate this rationale because it explains the asymmetry in how `dms-hand` dispatches Phase 1.5 vs Phases 1/2/3/4.
- Frontmatter `readonly: false` is correct for `dms-hand` (it must mutate four tracking files). `is_background: false` is correct (the user wants the cycle to complete and stop, not run in the background). `model: inherit` is the safe default.
- Cursor 2.5+ nested launches: `dms-hand` does not need nested launches; it dispatches `loremaster` once per cycle. But the fact that Cursor SUPPORTS the pattern means future expansions (e.g., `dms-hand` spawning quality-guardian after dm-registrar) are feasible.
