...
A worked end-to-end run from "queue has `001|nextjs-guardian` at top" to "Guardian registered with Dungeon Master's roster." Every file write is narrated.
...
The four tracking files look like this when `dms-hand` is invoked:

- `proposed-guardians-queue.md`: 231 rows in body. Top row: `001|nextjs-guardian`.
- `proposed-guardians-in-process.md`: empty.
- `proposed-guardians-completed.md`: empty.
- `proposed-guardians-backlog.md`: tier 1 has `### [ ] 1. nextjs-guardian` with full metadata block.

The user says: "Run the pipeline."

## Step 1: Pick

`dms-hand` reads `proposed-guardians-queue.md`. Top row of body: `001|nextjs-guardian`.
...
`dms-hand` uses StrReplace on `proposed-guardians-queue.md` to remove the line `001|nextjs-guardian\n`. After the edit, the new top row is `002|cursor-ide-guardian`. Queue body row count drops from 231 to 230.

## Step 2b: Update queue frontmatter

`dms-hand` updates the queue's YAML frontmatter:
- `totals.rows: 231` -> `totals.rows: 230`
- `date_updated: 2026-05-20`
- `last_updated_by: dms-hand`

## Step 2c: Append row to in-process

`dms-hand` writes `001|nextjs-guardian\n` to `proposed-guardians-in-process.md`. File now contains exactly one non-blank line.

The state machine is now in the `In-flight` state. Move-before-work invariant holds.

## Step 3: Backlog lookup

`dms-hand` opens `proposed-guardians-backlog.md` and searches for `### [ ] 1. nextjs-guardian`. Found at line 47.
...
- `nextjs-guardian` not present in `ai-tools/skills/dungeon-master/SKILL.md` -- OK.
- `nextjs-guardian` not present in `proposed-guardians-completed.md` -- OK.
...
`dms-hand` loads `ai-tools/skills/command-center/SKILL.md` and follows its instructions. The skill:
...
2. Writes a populated brief to `ai-tools/command-briefs/nextjs-guardian-command-brief.md` with:
   ...
   - Guardian half populated from the Purpose and `dms-hand`'s pre-filled context.
   - Weapon half with REFERENCE MATERIAL containing the 5 search queries and proposed authoritative sources.

Wall clock: 1m 23s.

`dms-hand` verifies the brief exists and is non-empty. OK. Proceed to Phase 1.5.
...
## Step 6: Phase 1.5 (loremaster)

`dms-hand` calls the Task tool:

```
Task(
  subagent_type="loremaster",
  description="Research nextjs-weapon (deep)",
  prompt="""
You are Phase 1.5 of the Guild AI Tools Factory pipeline, invoked by `dms-hand` for:

- Guardian: `nextjs-guardian`
...
`loremaster` runs for 12m 45s and emits its handoff line:
...
`dms-hand` loads `ai-tools/skills/weapon-forge/SKILL.md` and follows its instructions. The skill:
...
## Step 8: Phase 3 (guardian-creator)

`dms-hand` loads `ai-tools/skills/guardian-creator/SKILL.md` and follows its instructions. The skill:

1. Reads the Command Brief.
2. Walks the Weapon folder recursively to enumerate files for the References section.
3. Writes the Guardian file at `ai-tools/agents/nextjs-guardian.md` with:
...
`dms-hand` verifies the Guardian file. OK. Proceed to Phase 4.

## Step 9: Phase 4 (dm-registrar)

`dms-hand` loads `ai-tools/skills/dungeon-master-registrar/SKILL.md` and follows its instructions. The skill:

1. Adds a row to the Roster table in `ai-tools/skills/dungeon-master/SKILL.md`:
   ```
   ...
   ```
2. Writes `ai-tools/skills/dungeon-master/guides/nextjs-guardian.md` from `templates/guide-template.md`, substituting placeholders.
3. Increments the "N Guardians registered" footnote from 17 to 18.

Wall clock: 38s.

`dms-hand` verifies the roster row, the guide file, and the footnote increment. OK. Proceed to close-out.
...
- SKILL.md: OK
- Guardian file: OK
- Dungeon Master's roster row + guide: OK

### Step 10b: Delete in-process row

`dms-hand` removes `001|nextjs-guardian\n` from `proposed-guardians-in-process.md`. File is now empty.

### Step 10c: Append completed row

`dms-hand` appends to `proposed-guardians-completed.md`:
...
`dms-hand` edits `proposed-guardians-backlog.md`: `### [ ] 1. nextjs-guardian` -> `### [x] 1. nextjs-guardian`.

Wall clock for close-out: 5s.

## Step 11: Final report

`dms-hand` emits the canonical report (see `templates/final-report.md` for the worked example with these numbers).

## Ending state

- `proposed-guardians-queue.md`: 230 rows. Top row: `002|cursor-ide-guardian`. Frontmatter shows `totals.rows: 230`, `last_updated_by: dms-hand`.
- `proposed-guardians-in-process.md`: empty.
- `proposed-guardians-completed.md`: 1 row: `001|nextjs-guardian|completed|2026-05-20|...`.
- `proposed-guardians-backlog.md`: tier 1 has `### [x] 1. nextjs-guardian`.
...
- `ai-tools/skills/dungeon-master/SKILL.md` (with new roster row)
- `ai-tools/skills/dungeon-master/guides/nextjs-guardian.md`

`dms-hand` ends with the stop line. The orchestrator decides whether to invoke `dms-hand` again for `002|cursor-ide-guardian` or pause for human review.
