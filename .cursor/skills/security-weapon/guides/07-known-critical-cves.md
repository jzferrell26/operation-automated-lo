...
This guide tracks CVEs whose remediation is **upgrade now** rather than "patch in code." It complements `06-cve-tracker.md` — that guide is the live patch matrix the Guardian skims on every run; this guide is the deeper "here is what each CVE actually does and how the Guardian detects it" reference. Read this when a `pnpm audit` or `npm audit` finding lands on a CVE in the catalog below.
...
The Guardian reads `06-cve-tracker.md` on every run and this guide on demand. Both stay current via a manual quarterly refresh. The owner of that refresh follows this routine:
...
Every project this Guardian audits should run, at minimum:
...
4. Audit report templates pick up the new check on next Guardian invocation.
