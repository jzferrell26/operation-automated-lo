...
**Observed green on a GitHub runner.** On PR #66, GitHub Actions run `35056178244`, the `campaign-command-postgres` job passed in 55 seconds on its first attempt, and `preview-smoke` passed with the new job in its `needs:` list. The pre-existing Supabase-backed `database` job passed unchanged in 3 minutes 1 second, confirming the new job did not destabilize it. All seven checks on the PR are green.
