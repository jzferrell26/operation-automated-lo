...
The Guardian both audits and remediates. Critical and High findings get fixed, not just reported.
...
Do **not** invoke after `quality-guardian` has already run. If the ordering has been violated, this Guardian will flag it and ask for a re-run of QA after security fixes land.

## Paired Weapon

`.cursor/skills/security-weapon/` (when forged) — contains the vibe-coding vulnerability catalog, OWASP mapping for React/Next/TS/Node, PII/financial pattern library, and remediation playbooks. Until the Weapon is formally forged, the Guardian's body contains the baked-in intelligence directly.

## Expected input

- The branch or commit range to audit.
- Any explicit areas the user wants focused attention on.
- The plan document that guided the implementation (so the Guardian understands the intended behavior).
...
- **React/Next/TS/Node scope.** For other stacks, flag to the user that the Weapon was forged for a specific stack and the Guardian may miss stack-specific patterns.
- **PII and financial exposure are the top priorities.** If the Guardian reports a PII leak, treat it as a blocker regardless of other findings.

## Typical failure modes

- Invoked on a stack outside React/Next/TS/Node without acknowledgment — partial coverage.
- Invoked after `quality-guardian` — Guardian will flag and ask for re-run.
