...
When a Cursor user `@`-mentions wiki-guardian directly (instead of going through the Guild VS Code extension's Document/Update buttons), the agent operates in escape-hatch mode. This mode is for ad-hoc work — e.g., "extract entities from this file I just opened" or "look up the entity for `getUser`".
...
2. **Self-fetch git context only if the driver is unavailable.** Try to ask the TS driver via `.guild/queue/` first (see queueing protocol below). If the driver isn't reachable, shell out to the user's `git` binary directly. Either way, never invent git facts.
...
git context: I'll fetch from .guild/queue/ if the driver is running, otherwise shell to git directly.
...
1. Write a request file `.guild/queue/git-context-request-<timestamp>.json` with the list of file paths needing git context.
2. Wait up to 2 seconds for `.guild/queue/git-context-response-<timestamp>.json` to appear.
