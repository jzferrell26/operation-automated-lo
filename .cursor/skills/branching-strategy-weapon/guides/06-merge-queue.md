...
- If the Merge Queue setup requires changes to GitHub Actions workflows: route to `devops-guardian`.
- If the Merge Queue requires branch protection ruleset changes: route to `github-repo-health-guardian`.
- `branching-strategy-guardian` owns the decision of whether to use a merge queue; the other Guardians own the configuration.
