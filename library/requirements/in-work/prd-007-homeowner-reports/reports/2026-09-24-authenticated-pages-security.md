...
The final provider-contract check also verified HighLevel's current v3 documentation at `https://marketplace.gohighlevel.com/docs/ghl/contacts/update-contact/`. Its DND example uses lowercase channel names. `server/homeowners/highlevel.ts:117-127` now recognizes lowercase and legacy case variants while still requiring both email and SMS channels, global DND false, and every supplied channel status explicitly inactive. Contradictory duplicate keys, partial settings and active/unknown statuses remain blocked before any write. Adapter regression tests exercise both accepted representations and the refusal cases. This change does not enable delivery or provide a live connection.

## Walkthrough capture follow-up

The final mobile screenshot correction is confined to `tests/browser/review/guided-setup.walkthrough-captures.spec.ts`. It reuses the existing scroll preparation after the mobile viewport is established, then runs the same accessibility, layout, target-size and reference-image checks. It changes no application route, persisted record, credential, permission, external request, screenshot tolerance or golden image. The test still reaches each state through the real authenticated workflow. No new security exposure was identified in this test-only diff; the following quality rerun qualifies its capture behavior and the complete release gate remains required.
