...
A Weapon is only as credible as the research behind it. This protocol keeps research rigorous, attributable, and maintainable over time.
...
4. A query for the adjacent topics the Guardian might encounter
...
<Your annotations on how this fits the Guardian's job>
```
...
2. Handle the common cases the Guardian will see.
3. Recognize when the Guardian is in unfamiliar territory (so it can escalate).

As a rough heuristic: three to seven research files per Weapon is typical. Less suggests the research was superficial; more suggests the Weapon's scope is too broad and should have been split into two Guardians at brief time.
...
- "Which internal channel should seo-guardian post to when it flags a blocker?"
