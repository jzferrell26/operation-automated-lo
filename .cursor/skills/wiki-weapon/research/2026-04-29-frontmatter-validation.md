...
## Summary
Wiki-guardian's frontmatter is the typed contract every page must satisfy. The validation stack is `gray-matter` (parse YAML out of markdown) + `zod` (validate the parsed object against a typed schema), combined idiomatically by `zod-matter`. This gives runtime validation, static type inference, and clean error reporting in one. For lint-mode pass/fail reports, Zod's `safeParse` returns `{ success: false, error: ZodError }` with a structured issue list — exactly what the lint mode needs to surface as wiki-guardian's `lint_findings`.
...
- Common gotchas:
  ...
  - Wikilinks in YAML: `related: [[entities/foo]]` is invalid YAML. Use array-of-strings `related: ["[[entities/foo]]"]` or write the wikilinks in the body, not the frontmatter. (Claude-obsidian uses the second form for relationships in YAML — a quoted string per link.)

## Recommended approach for wiki-guardian

Define one Zod schema per page type (`entity`, `concept`, `decision`, `comparison`, `question`, `meta`) in `references/frontmatter-schema.md` (rendered as code blocks) AND in code in the TS driver. The schema lives in the driver, not in the agent — wiki-guardian writes pages and the driver lints them. For wiki-guardian's purposes, the agent treats the schema as a contract: emit fields exactly per the table.
...
For lint mode, the driver runs `EntitySchema.safeParse(graymatterOut.data)` per page; on failure, `error.issues` becomes the lint finding's payload. Wiki-guardian itself doesn't import Zod — the driver does. But wiki-guardian must follow the contract exactly when authoring; this research note tells the Guardian WHAT shape to write.

For YAML date strings (avoid the Date-coercion gotcha): always quote dates: `created: "2026-04-29"`. Document this in the Guardian guide.
...
## Open questions / gaps
- Should `path` be enforced as POSIX (forward-slash) or platform-native? Recommend POSIX always — Windows TS driver normalizes before writing. Cross-platform repos break otherwise.
