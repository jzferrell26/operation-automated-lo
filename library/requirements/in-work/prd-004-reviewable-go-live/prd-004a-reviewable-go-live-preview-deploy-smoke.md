...
| ~~Real-Postgres round trips are not enforced by CI~~ | Resolved 2026-09-16 (CI gate pending its first run) | The suite now executes. It never needed Supabase local: the migrations reference no Supabase object and only require a superuser connection, which Supabase local's `postgres` role lacks. `packages/db/test/campaign-command.integration.test.mjs` passes 9 of 9 against stock PostgreSQL 17. A new `campaign-command-postgres` job is written to enforce it on a digest-pinned `postgres:17-bookworm` service container, but that job has not yet executed on a GitHub runner, so the enforcement is locally rehearsed rather than observed green. The canonical `pnpm test:db` gate still does not run this suite. See `GGL-B16` in [`EXECUTION_LEDGER.md`](../../../../EXECUTION_LEDGER.md). |
