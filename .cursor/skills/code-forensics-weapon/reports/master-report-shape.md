...
This folder is where the final master report and supporting deliverables land when the Guardian completes a case. The `scripts/build_master_report.js` Node builder writes `{ProjectName}_Forensic_Report.docx` into a case's `forensic-output/` folder, then LibreOffice headless converts it to PDF.
...
- Lessons learned that should be folded back into the next iteration of the Weapon
