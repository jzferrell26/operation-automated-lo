...
- The "Replaces (in current code)" section in each component brief is the
  Weapon's extension — shadcn/ui doesn't have it because it's the greenfield
  source; our Guardian operates on existing products and must make migration
  explicit.
- When a product already uses shadcn/ui, the Guardian's component briefs should
