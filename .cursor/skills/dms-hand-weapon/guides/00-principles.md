...
The non-negotiable rules that govern every `dms-hand` invocation. Read this guide on every cycle; the rest of the weapon assumes you have internalized these principles.

## The foreman vs craftsman boundary

`dms-hand` is the factory foreman. It does not research, design, code, audit, or author any product-domain content. Its job is to dispatch the worker phases in the correct order, watch each phase report success, and update the tracking files between phases.

This separation is intentional and is rooted in published Cursor engineering practice. As Cursor's own engineering team writes on self-driving codebases (see `research/external/2026-05-20-cursor-self-driving-codebases.md`):
...
The five-Guardian-plus-orchestrator split (session-zero proposes, command-center interviews, loremaster researches, weapon-forge builds, guardian-creator writes, dm-registrar registers, dms-hand orchestrates) is the implementation of this rule.

If you ever feel tempted to "just write a bit of the SKILL.md" or "just summarize the research", STOP. That work belongs to the worker phase. Crossing the boundary corrupts the separation of concerns that makes the pipeline auditable.

## The move-before-work invariant

The single most important rule in this weapon. Stated formally:

> Before invoking ANY phase, the queue row being processed must already be deleted from `proposed-guardians-queue.md` AND appended to `proposed-guardians-in-process.md`.

The invariant exists because there is no OS-level lock on a markdown file. If two `dms-hand` invocations were to read the queue concurrently, both would see the same top row. The move-before-work pattern is the agent-layer equivalent of an atomic rename: by the time phase work begins, the row is verifiably in `in-process` and not in `queue`, so a second invocation entering at the same moment would see a different top row OR refuse to start (because the in-process file is non-empty).
...
`dms-hand`'s markdown-row move is the moral equivalent. The semantics are the same: claim before work, never work before claim.
...
`dms-hand` is the root of a planner-worker hierarchy. The worker phases never talk to each other directly. Each worker hands its output back to `dms-hand` via the on-disk artifact (Command Brief, research folder, weapon folder, Guardian file, roster row). `dms-hand` is the only entity that reads multiple workers' outputs.
...
3. Adding a sixth phase (or removing one) only changes `dms-hand`'s phase list, not the workers.
...
For `dms-hand`'s single-cycle scope, "handoff reports" are the artifacts on disk.

## Markdown files as a state machine

The four tracking files are not loose documentation. They are a finite state machine encoded in markdown. Each Guardian proposal has exactly four observable states:

| State | Where the row lives |
|---|---|
| Proposed | `proposed-guardians-backlog.md` (with `### [ ] N. name`) AND `proposed-guardians-queue.md` (with `NNN\|name`) |
| In-flight | `proposed-guardians-backlog.md` (still `[ ]`) AND `proposed-guardians-in-process.md` (with `NNN\|name`) |
| Completed | `proposed-guardians-backlog.md` (now `[x]`) AND `proposed-guardians-completed.md` (with `NNN\|name\|completed\|YYYY-MM-DD\|...`) |
| Failed mid-cycle | `proposed-guardians-backlog.md` (still `[ ]`) AND `proposed-guardians-in-process.md` (with `NNN\|name\|failed:<phase>`) |
...
This state-machine framing is the right mental model for the entire weapon. As Phoenix argues (`research/external/2026-05-20-markdown-files-as-state-machines.md`):
...
The tracking files ARE the checkpoint files. `dms-hand` survives context compaction by re-reading them.

## The ten non-negotiables

These are stated tersely. The rest of the weapon expands each.
...
7. **Stop after dm-registrar.** Emit the final report, then end the run. Do not loop.
8. **Refuse to start a new cycle if `proposed-guardians-in-process.md` already contains a non-blank row.**
