...
Three tools, each deterministic, each cheap to run as Phase 1 automation before the Guardian spends judgment-cycles:
...
## What the tools DON'T catch (Guardian judgment required)
...
- `scripts/scan.sh` runs the three tools above and drops JSON reports into a local gitignored scratch dir (e.g. `.scan-output/`) — the Guardian reads them, dedupes, and promotes findings into its own report.
- The Guardian's value is concentrated in the "DON'T catch" list — `guides/00-principles.md` says so explicitly so Guardian time is spent on judgment, not on re-running grep.
