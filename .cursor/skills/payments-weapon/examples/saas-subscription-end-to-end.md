...
## Cross-Guardian handoffs needed

- `db-guardian` — schema for `subscriptions`, `users.entitlements`, `processed_webhook_events`. Migration.
- `security-guardian` — secret storage in env, RBAC on `/api/portal`, retention of webhook payloads.
- `react-guardian` — the React-side `useCheckout` hook that POSTs to `/api/checkout` and handles the redirect.
- `library-guardian` — the PRD that defined "what does Gold give you?" — should map 1:1 to `entitlementsFor(lookupKey)`.
