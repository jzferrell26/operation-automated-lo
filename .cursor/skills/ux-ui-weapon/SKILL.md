...
```
guild-shared/brands/guild/          <- parent brand (fallback for all consumers)
guild-shared/brands/<sub-brand>/     <- sub-brand overlay (only overrides)
```
...
1. Start with `brands/guild/brand_kit/colors_and_type.css` for the canonical color + type tokens.
...
Trigger when a user or another Guardian:
