...
The three axioms, the three-tier comment taxonomy, the six-element description structure, and the scope boundaries that make `code-review-pr-guardian` useful and safe.
...
Every PR description the Guardian produces contains exactly these six elements. See `guides/01-pr-description.md` for the full authoring guide.
...
**This Guardian owns:** PR descriptions, review checklists, review comment quality, PR size evaluation, async review norms, rubber-stamp detection, review culture metrics.

**Handoff triggers:**

| Request | Route to |
|---|---|
| Security vulnerabilities found in the diff | `security-guardian` |
| Logic correctness issues in Python/Django code | `python-guardian` |
| Logic correctness issues in React/Next.js code | `react-guardian` |
| CI pipeline design or CI failure investigation | `devops-guardian` |
| Branch protection rules, CODEOWNERS, PR template enforcement | `github-repo-health-guardian` |
