...
For **authoring** wikilinks: always emit the full repo-relative form `[[entities/foo]]` (with the `entities/` prefix). This avoids the shortest-path ambiguity entirely and makes grep-based scans trivial. The TS driver can compress to bare names at render time if desired. For **lint-mode dead-link detection**, implement this algorithm (in the TS driver, not the agent — the agent reports gaps it noticed in-flight, not the global lint sweep):
...
For wiki-guardian itself in `document`/`update` modes: when a wikilink in the chunk references an entity not yet authored, emit a `gap` in the response payload with `{entity, referenced_in: file:line, reason}` per the brief. Don't try to resolve it in-flight — the driver runs reconciliation.
...
## Open questions / gaps
- For Cursor's preview, does shortest-path resolution apply, or does Cursor only render `[[bare-name]]` literally? Tested briefly — Cursor renders wikilinks as plain text in markdown preview unless an extension is installed. Recommend wiki-guardian treat wikilinks as machine-greppable internal anchors first, human-readable second; the TS driver can offer a "render-as-Obsidian" preview mode.
- Should wiki-guardian dedupe near-name entities (e.g., `getUser` vs `get-user`)? Recommend kebab-case for filenames AND camelCase preserved in body — this avoids 90% of dedup ambiguity. Document in the Guardian guide.
